...
## 1. 需求
动画对比对称加密(AES)与非对称加密(RSA)的密钥使用方式。

## 2. 专业思考
加密技术。动画两态：对称(AES 同密钥加解密、快但密钥分发难)、非对称(RSA 公钥加密私钥解密、解决分发但慢)。强调实际 HTTPS 用 RSA 交换对称密钥 + AES 传数据，兼顾安全与效率。
...
<header class="topbar">
  ...
  <span class="title">对称与非对称加密</span>
  <span class="subtitle">AES / RSA</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>对称</h4><div class="v">同密钥加解密</div></div><div class="card s3"><div class="num">2</div><h4>非对称</h4><div class="v">公钥加密私钥解</div></div><div class="card s4"><div class="num">3</div><h4>速度</h4><div class="v">对称快</div></div><div class="card "><div class="num">4</div><h4>结合</h4><div class="v">HTTPS 混合</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">对称与非对称加密</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放对比两者</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "加密目的", "body": "<strong>加密</strong>把明文变密文，防止信息被窃听篡改，保障机密与完整。", "points": {"title": "目的", "items": ["<strong>机密</strong>性", "<strong>完整</strong>性", "<strong>认证</strong>", "<strong>防窃</strong>"]}}, {"tag": "对称", "name": "对称加密", "body": "<strong>对称加密(AES)</strong>：加解密用<strong>同一密钥</strong>，速度快，但密钥分发困难。", "points": {"title": "对称", "items": ["<strong>同密钥</strong>", "<strong>快</strong>", "<strong>AES</strong>", "<strong>分发难</strong>"]}, "code": "明文 + key → 密文 (同key解)"}, {"tag": "非对称", "name": "非对称加密", "body": "<strong>非对称(RSA)</strong>：<strong>公钥加密、私钥解密</strong>，解决密钥分发，但较慢。", "points": {"title": "非对称", "items": ["<strong>公钥</strong>加密", "<strong>私钥</strong>解密", "<strong>RSA</strong>", "<strong>慢</strong>"]}, "code": "公钥加密 → 私钥解密"}, {"tag": "结合", "name": "混合加密", "body": "实际如 <strong>HTTPS</strong>：用 RSA 安全交换对称密钥，再用 AES 高速传数据。", "points": {"title": "结合", "items": ["<strong>RSA</strong>：换密钥", "<strong>AES</strong>：传数据", "<strong>HTTPS</strong>", "<strong>高效安全</strong>"]}}];
let stepIdx=0;
const CRYPTO={t:0};
function sceneInit(){CRYPTO.t=0;}
function sceneReset(){CRYPTO.t=0;}
function sceneUpdate(dt){CRYPTO.t+=dt*speed;if(CRYPTO.t>6)CRYPTO.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 const s=Math.floor(CRYPTO.t/3)%2;
 drawText(s?'非对称加密 RSA：公钥加密→私钥解密(慢,密钥分发)':'对称加密 AES：同一密钥加解密(快)',W/2,18,'#74b9ff',12);
 const cx=W/2,cy=H/2;
 if(s){ctx.fillStyle='#1e3a5f';ctx.fillRect(cx-50,cy-20,100,40);drawText('明文',cx,cy,'#fbbf24',10);ctx.fillStyle='#34d399';ctx.fillRect(cx-50,cy+20,100,30);drawText('公钥加密→密文',cx,cy+35,'#06281c',10);ctx.fillStyle='#9ab0d0';ctx.fillRect(cx-50,cy+60,100,20);drawText('私钥解密',cx,cy+70,'#06281c',10);}
 else{ctx.fillStyle='#1e3a5f';ctx.fillRect(cx-50,cy-20,100,40);drawText('明文+密钥',cx,cy,'#fbbf24',10);ctx.fillStyle='#f87171';ctx.fillRect(cx-50,cy+20,100,30);drawText('同钥→密文',cx,cy+35,'#fff',10);ctx.fillStyle='#38bdf8';ctx.fillRect(cx-50,cy+60,100,20);drawText('同钥→还原',cx,cy+70,'#fff',10);}
 drawText('HTTPS=非对称交换密钥+AES 传数据',W/2,H-20,'#9ab0d0',11);}
