...
## 1. 需求
动画展示神经网络三层结构与前向传播的信号流动。

## 2. 专业思考
神经网络基础。动画三层节点(输入/隐藏/输出)及随时间波动的连接权重，示意前向传播。强调神经元=加权求和+激活(非线性)、多层叠加可拟合复杂函数、前向得预测后与标签比对再反向传播调参。
...
<header class="topbar">
  ...
  <span class="title">神经网络 · 前向传播</span>
  <span class="subtitle">输入→隐藏→输出</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>结构</h4><div class="v">层+神经元</div></div><div class="card s3"><div class="num">2</div><h4>权重</h4><div class="v">加权求和</div></div><div class="card s4"><div class="num">3</div><h4>激活</h4><div class="v">非线性变换</div></div><div class="card "><div class="num">4</div><h4>前向</h4><div class="v">逐层传递</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">神经网络 · 前向传播</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察信号流</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "网络结构", "body": "<strong>神经网络</strong>由<strong>输入层、隐藏层、输出层</strong>的神经元(节点)及连接权重组成。", "points": {"title": "结构", "items": ["<strong>输入层</strong>", "<strong>隐藏层</strong>", "<strong>输出层</strong>", "<strong>节点</strong>"]}}, {"tag": "权重", "name": "加权求和", "body": "每个神经元对输入<strong>乘权重求和</strong>，再加偏置。", "points": {"title": "加权", "items": ["<strong>Σw·x</strong>", "<strong>偏置</strong>b", "<strong>线性</strong>", "<strong>组合</strong>"]}, "code": "z = Σ wᵢxᵢ + b"}, {"tag": "激活", "name": "激活函数", "body": "求和后经<strong>激活函数</strong>(如 sigmoid/ReLU)引入非线性，使网络能拟合复杂函数。", "points": {"title": "激活", "items": ["<strong>sigmoid</strong>", "<strong>ReLU</strong>", "<strong>非线性</strong>", "<strong>关键</strong>"]}}, {"tag": "前向", "name": "前向传播", "body": "信号从输入层<strong>逐层前向传递</strong>至输出层，得到预测，再与标签比较反向调参。", "points": {"title": "前向", "items": ["<strong>输入</strong>→输出", "<strong>逐层</strong>", "<strong>预测</strong>", "<strong>反传</strong>调参"]}}];
let stepIdx=0;
const NN={t:0};
function sceneInit(){NN.t=0;}
function sceneReset(){NN.t=0;}
function sceneUpdate(dt){NN.t+=dt*speed;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('神经网络：输入层→隐藏层→输出层，前向传播',W/2,20,'#74b9ff',12);
 const layers=[[3,2],[4,2],[3,3]];
 const xs=[W*0.2,W*0.5,W*0.8];
 for(let l=0;l<3;l++){const cnt=layers[l][0];for(let i=0;i<cnt;i++){const y=H/2-(cnt-1)*30/2+i*30;ctx.fillStyle='#1e3a5f';ctx.beginPath();ctx.arc(xs[l],y,14,0,7);ctx.fill();ctx.strokeStyle='#3b82f6';ctx.stroke();if(l<2){for(let j=0;j<layers[l+1][0];j++){const y2=H/2-(layers[l+1][0]-1)*30/2+j*30;ctx.strokeStyle='rgba(116,185,255,'+(0.2+0.2*Math.sin(NN.t*3+i+j))+')';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(xs[l],y);ctx.lineTo(xs[l+1],y2);ctx.stroke();}}}}
 drawText('权重 w 加权求和→激活→逐层传递',W/2,H-22,'#9ab0d0',11);}
