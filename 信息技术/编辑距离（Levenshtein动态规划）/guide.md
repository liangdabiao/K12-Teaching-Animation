...
## 1. 需求
动画展示编辑距离的动态规划填表过程（KITTEN→SITTING），逐行揭示状态值。

## 2. 专业思考
编辑距离是经典 DP。动画以 KITTEN→SITTING 为例，逐行填充 dp 表：第 0 行/列为与空串的距离，每格取 左/上/对角 最小+1（对角在字符相同时不+1）。揭示到当前行高亮，最终右下角=3。强调状态定义 dp[i][j]=前缀距离、边界处理、最优子结构。延伸：Needleman–Wunsch 用于生物序列比对（允许不同罚分）。
...
<header class="topbar">
  ...
  <span class="title">编辑距离 · 动态规划</span>
  <span class="subtitle">插入/删除/替换</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>定义</h4><div class="v">最小编辑代价</div></div><div class="card s3"><div class="num">2</div><h4>操作</h4><div class="v">插入/删除/替换</div></div><div class="card s4"><div class="num">3</div><h4>DP</h4><div class="v">二维表递推</div></div><div class="card "><div class="num">4</div><h4>应用</h4><div class="v">模糊匹配/校对</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">编辑距离 · 动态规划</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察填表</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "什么是编辑距离", "body": "<strong>编辑距离（Levenshtein）</strong>指把一个字符串变成另一个所需的最少<strong>插入、删除、替换</strong>操作次数。", "points": {"title": "操作", "items": ["<strong>插入</strong>：+1", "<strong>删除</strong>：+1", "<strong>替换</strong>：+1", "<strong>相同</strong>：0"]}, "code": "dist = 最少 插入/删除/替换 次数"}, {"tag": "递推", "name": "DP 递推式", "body": "设 dp[i][j] 为 A 前 i 字符到 B 前 j 字符的距离：若 A[i]==B[j] 则 dp[i][j]=dp[i-1][j-1]，否则取 插入/删除/替换 三者最小 +1。", "points": {"title": "状态", "items": ["<strong>dp[i][j]</strong>：前缀距离", "<strong>相同</strong>：对角", "<strong>不同</strong>：min+1", "<strong>边界</strong>：空串=j/i"]}, "code": "if A[i]==B[j]: dp=dp[i-1][j-1]\nelse: dp=1+min(左,上,对角)"}, {"tag": "填表", "name": "填表过程", "body": "先填第 0 行/列（与空串的距离），再逐格按递推式填充，最终 <strong>右下角</strong>即整体编辑距离。", "points": {"title": "顺序", "items": ["<strong>第0行/列</strong>：0,1,2…", "<strong>逐行</strong>：左→右", "<strong>逐列</strong>：上→下", "<strong>终点</strong>：右下角"]}}, {"tag": "应用", "name": "应用", "body": "编辑距离用于<strong>拼写纠错、DNA 序列比对、模糊搜索、 plagiarism 检测</strong>等。", "points": {"title": "用途", "items": ["<strong>拼写检查</strong>", "<strong>序列比对</strong>", "<strong>模糊匹配</strong>", "<strong>相似度</strong>度量"]}}];
let stepIdx=0;
const ED={t:0,A:'KITTEN',B:'SITTING',dp:[]};
function computeDP(){const m=ED.A.length,n=ED.B.length;const d=[];for(let i=0;i<=m;i++){d[i]=[];for(let j=0;j<=n;j++){if(i===0)d[i][j]=j;else if(j===0)d[i][j]=i;else{const c=ED.A[i-1]===ED.B[j-1]?0:1;d[i][j]=Math.min(d[i-1][j]+1,d[i][j-1]+1,d[i-1][j-1]+c);}}}ED.dp=d;}
function sceneInit(){computeDP();ED.t=0;}
function sceneReset(){computeDP();ED.t=0;}
function sceneUpdate(dt){ED.t+=dt*speed;if(ED.t>(ED.A.length+1))ED.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('编辑距离（Levenshtein）：KITTEN → SITTING',W/2,20,'#74b9ff',14);
 const m=ED.A.length,n=ED.B.length;const cell=Math.min(46,(W-120)/(n+2));const x0=70,y0=60;const filled=Math.floor(ED.t);
 // header B
 for(let j=0;j<=n;j++){const x=x0+(j+1)*cell;drawText(j===0?'':ED.B[j-1],x+cell/2,y0,x===0?'#9ab0d0':'#fbbf24',12);}
 drawText('',x0,y0,'#9ab0d0',12);
 for(let i=0;i<=m;i++){const y=y0+(i+1)*cell;drawText(i===0?'':ED.A[i-1],x0+cell/2,y,'#fbbf24',12);
  for(let j=0;j<=n;j++){const x=x0+(j+1)*cell;const reveal=(i<=filled);const cur=(i===filled);
   ctx.fillStyle=cur?'#34d399':(reveal?'#1e3a5f':'#0d1117');ctx.strokeStyle='#243a5e';ctx.lineWidth=1;ctx.fillRect(x,y,cell,cell);ctx.strokeRect(x,y,cell,cell);
   if(reveal)drawText(String(ED.dp[i][j]),x+cell/2,y+cell/2,cur?'#06281c':'#fbbf24',11);}
 }

 drawText('右下角 = '+ED.dp[m][n]+'（最少编辑次数）',W/2,H-18,'#9ab0d0',12);}
