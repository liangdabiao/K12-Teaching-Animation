...
## 1. 需求
动画展示 DNS 递归与迭代查询的完整链路（浏览器→本地→根→TLD→权威）。

## 2. 专业思考
DNS 域名解析。动画五阶段依次点亮：浏览器→本地DNS(递归)、本地→根、根→TLD、TLD→权威、返回IP逐级回传。强调递归(本地代客户查)与迭代(服务器间逐级指引)的区别、分布式分层结构、缓存加速。
...
<header class="topbar">
  ...
  <span class="title">DNS 域名解析</span>
  <span class="subtitle">递归 + 迭代查询</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>作用</h4><div class="v">域名→IP</div></div><div class="card s3"><div class="num">2</div><h4>递归</h4><div class="v">本地代查</div></div><div class="card s4"><div class="num">3</div><h4>迭代</h4><div class="v">逐级指引</div></div><div class="card "><div class="num">4</div><h4>缓存</h4><div class="v">加速</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">DNS 域名解析</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察查询链</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "DNS 作用", "body": "<strong>DNS(域名系统)</strong>把人类易记的<strong>域名解析为 IP 地址</strong>。", "points": {"title": "作用", "items": ["<strong>域名</strong>→IP", "<strong>分布式</strong>", "<strong>分层</strong>", "<strong>53</strong>端口"]}}, {"tag": "递归", "name": "递归查询", "body": "主机向<strong>本地 DNS</strong>发起<strong>递归查询</strong>：本地服务器负责替客户查到结果。", "points": {"title": "递归", "items": ["<strong>主机→本地</strong>", "<strong>本地代查</strong>", "<strong>返回</strong>结果", "<strong>客户省心</strong>"]}, "code": "client → 本地DNS(递归)"}, {"tag": "迭代", "name": "迭代查询", "body": "本地 DNS 向<strong>根、TLD、权威</strong>服务器发起<strong>迭代查询</strong>，逐级获得指引直至拿到 IP。", "points": {"title": "迭代", "items": ["<strong>根</strong>：指引", "<strong>TLD</strong>：指引", "<strong>权威</strong>：给IP", "<strong>逐级</strong>"]}, "code": "本地 → 根 → TLD → 权威(迭代)"}, {"tag": "缓存", "name": "缓存加速", "body": "各级服务器<strong>缓存</strong>解析结果，有效期内直接返回，加速访问、减轻负载。", "points": {"title": "缓存", "items": ["<strong>缓存</strong>结果", "<strong>有效期</strong>", "<strong>加速</strong>", "<strong>减负</strong>"]}}];
let stepIdx=0;
const DNS={t:0};
function sceneInit(){DNS.t=0;}
function sceneReset(){DNS.t=0;}
function sceneUpdate(dt){DNS.t+=dt*speed;if(DNS.t>8)DNS.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 const steps=['① 浏览器→本地DNS(递归查询)','② 本地→根服务器','③ 根→TLD服务器','④ TLD→权威服务器','⑤ 返回IP，逐级回传'];
 const s=Math.floor(DNS.t/1.6)%5;
 drawText('DNS 域名解析：'+steps[s],W/2,20,'#74b9ff',12);
 const nodes=[['浏览器',W*0.15,H/2],['本地DNS',W*0.35,H/2],['根',W*0.55,H*0.3],['TLD',W*0.55,H*0.7],['权威',W*0.75,H/2]];
 const links=[[0,1],[1,2],[1,3],[1,4]];
 for(const l of links){ctx.strokeStyle='#243a5e';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(nodes[l[0]][1],nodes[l[0]][2]);ctx.lineTo(nodes[l[1]][1],nodes[l[1]][2]);ctx.stroke();}
 const active=s+1;for(let i=0;i<nodes.length;i++){ctx.fillStyle=i<=active?'#34d399':'#1e3a5f';ctx.beginPath();ctx.arc(nodes[i][1],nodes[i][2],18,0,7);ctx.fill();ctx.strokeStyle='#3b82f6';ctx.stroke();drawText(nodes[i][0],nodes[i][1],nodes[i][2],'#fbf4d0',10);}
 drawText('域名→IP：递归(本地代查)+迭代(逐级指引)',W/2,H-22,'#9ab0d0',11);}
