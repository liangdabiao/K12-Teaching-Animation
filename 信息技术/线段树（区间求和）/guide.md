...
## 1. 需求
动画展示线段树的建树与区间求和查询，高亮覆盖查询区间的节点。

## 2. 专业思考
线段树是区间查询利器。动画展示一棵 8 叶子线段树，查询 [2,5] 时高亮覆盖该区间的节点（绿色），并实时显示这些节点值之和。强调“完全覆盖即返回、部分重叠则分裂”的递归策略，使查询降到 O(log n)。可延伸：区间修改用懒标记（lazy tag）。
...
<header class="topbar">
  ...
  <span class="title">线段树 · 区间求和查询</span>
  <span class="subtitle">O(log n) 区间查询</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>结构</h4><div class="v">平衡二叉树存区间</div></div><div class="card s3"><div class="num">2</div><h4>建树</h4><div class="v">O(n) 自底向上</div></div><div class="card s4"><div class="num">3</div><h4>查询</h4><div class="v">O(log n) 覆盖节点</div></div><div class="card "><div class="num">4</div><h4>应用</h4><div class="v">区间最值/求和</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">线段树 · 区间求和查询</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察查询覆盖</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "线段树概念", "body": "<strong>线段树</strong>是用二叉树维护区间信息的数据结构，每个节点代表一个区间，存该区间的聚合值（如和、最值）。", "points": {"title": "结构", "items": ["<strong>节点</strong>：代表区间", "<strong>叶子</strong>：单元素", "<strong>内部</strong>：左右子合并", "<strong>聚合</strong>：和/最值"]}, "code": "node.区间 = [L,R]\nnode.val = f(左.val, 右.val)"}, {"tag": "建树", "name": "建树", "body": "自底向上建树：叶子存数组元素，内部节点值 = 左右子值之和（或最值）。建树时间 O(n)。", "points": {"title": "建树", "items": ["<strong>叶子</strong>：a[i]", "<strong>内部</strong>：left+right", "<strong>时间</strong>：O(n)", "<strong>空间</strong>：O(4n)"]}}, {"tag": "查询", "name": "区间查询", "body": "查询 [L,R]：从根递归，若节点区间完全在查询内则返回其值（被覆盖），否则分裂到左右子，直至完全覆盖或部分重叠。复杂度 O(log n)。", "points": {"title": "查询", "items": ["<strong>完全覆盖</strong>：直接返回", "<strong>部分重叠</strong>：递归左右", "<strong>无交</strong>：返回 0/∞", "<strong>复杂度</strong>：O(log n)"]}, "code": "query(node,L,R):\n  if 节点⊆[L,R]: return node.val\n  return query(左)+query(右)"}, {"tag": "应用", "name": "应用", "body": "线段树支持<strong>区间求和、区间最值、区间修改</strong>等，是算法竞赛与工程中的高频结构。", "points": {"title": "用途", "items": ["<strong>区间和</strong>", "<strong>区间最值</strong>", "<strong>区间更新</strong>", "<strong>二维</strong>可扩展"]}}];
let stepIdx=0;
const SEG={t:0,arr:[1,3,5,7,9,2,4,6],tree:[],qL:2,qR:5};
function buildSeg(){SEG.tree=new Array(15).fill(0);(function build(node,l,r){if(l===r){SEG.tree[node]=SEG.arr[l];return;}const m=(l+r)>>1;build(node*2,l,m);build(node*2+1,m+1,r);SEG.tree[node]=SEG.tree[node*2]+SEG.tree[node*2+1];})(1,0,7);}
function sceneInit(){buildSeg();SEG.t=0;}
function sceneReset(){buildSeg();SEG.t=0;}
function sceneUpdate(dt){SEG.t+=dt*speed;if(SEG.t>6)SEG.t=0;}
function queryNodes(node,l,r,res){if(SEG.qL<=l&&r<=SEG.qR){res.push(node);return;}const m=(l+r)>>1;if(SEG.qL<=m)queryNodes(node*2,l,m,res);if(SEG.qR>m)queryNodes(node*2+1,m+1,r,res);}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('线段树：区间求和查询 ['+SEG.qL+','+SEG.qR+']',W/2,22,'#74b9ff',14);
 const hi=[];queryNodes(1,0,7,hi);
 const pos=[];for(let lv=0;lv<4;lv++){const cnt=1<<lv;for(let i=0;i<cnt;i++){pos[(1<<lv)+i]={x:W*(i+0.5)/cnt,y:64+lv*68};}}
 for(let node=1;node<15;node++){const p=pos[node];if(!p)continue;ctx.fillStyle=hi.indexOf(node)>=0?'#34d399':'#1e3a5f';ctx.strokeStyle='#3b82f6';ctx.lineWidth=2;ctx.beginPath();ctx.arc(p.x,p.y,20,0,7);ctx.fill();ctx.stroke();drawText(SEG.tree[node],p.x,p.y,hi.indexOf(node)>=0?'#06281c':'#fbbf24',12);
  if(node*2<15&&pos[node*2]){ctx.strokeStyle='#243a5e';ctx.beginPath();ctx.moveTo(p.x,p.y+20);ctx.lineTo(pos[node*2].x,pos[node*2].y-20);ctx.stroke();}
  if(node*2+1<15&&pos[node*2+1]){ctx.strokeStyle='#243a5e';ctx.beginPath();ctx.moveTo(p.x,p.y+20);ctx.lineTo(pos[node*2+1].x,pos[node*2+1].y-20);ctx.stroke();}}
 const ay=H-40,cw=(W-40)/8;for(let i=0;i<8;i++){ctx.fillStyle=(i>=SEG.qL&&i<=SEG.qR)?'#34d399':'#1e3a5f';ctx.strokeStyle='#3b82f6';ctx.fillRect(20+i*cw,ay,cw-4,22);ctx.strokeRect(20+i*cw,ay,cw-4,22);drawText(SEG.arr[i],20+i*cw+cw/2-2,ay+11,(i>=SEG.qL&&i<=SEG.qR)?'#06281c':'#fbbf24',11);}
 drawText('覆盖节点和 = '+hi.reduce((a,n)=>a+SEG.tree[n],0),W/2,H-12,'#9ab0d0',12);}
