...
## 1. 需求
动画展示事务的 ACID 四特性：原子、一致、隔离、持久。

## 2. 专业思考
事务 ACID。动画四态：原子性(commit/rollback、全做或全不做)、一致性(约束满足、总额不变)、隔离性(并发事务互不干扰)、持久性(提交后落盘不丢)。强调 ACID 是数据库可靠性的基石，典型场景是银行转账。
...
<header class="topbar">
  ...
  <span class="title">事务与 ACID</span>
  <span class="subtitle">原子/一致/隔离/持久</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>A原子</h4><div class="v">全做或全不做</div></div><div class="card s3"><div class="num">2</div><h4>C一致</h4><div class="v">约束满足</div></div><div class="card s4"><div class="num">3</div><h4>I隔离</h4><div class="v">并发互不扰</div></div><div class="card "><div class="num">4</div><h4>D持久</h4><div class="v">提交后永久</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">事务与 ACID</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察四特性</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "事务概念", "body": "<strong>事务</strong>是数据库操作的逻辑单元，要么完整执行，要么全不执行。", "points": {"title": "概念", "items": ["<strong>逻辑单元</strong>", "<strong>ACID</strong>", "<strong>银行</strong>：转账", "<strong>原子</strong>"]}}, {"tag": "A", "name": "原子性 A", "body": "<strong>原子性</strong>：事务内操作要么全成功，要么<strong>全部回滚</strong>，无中间态。", "points": {"title": "A", "items": ["<strong>全做</strong>", "<strong>全不做</strong>", "<strong>回滚</strong>", "<strong>无半</strong>"]}}, {"tag": "C", "name": "一致性 C", "body": "<strong>一致性</strong>：事务前后数据库完整性约束始终满足。", "points": {"title": "C", "items": ["<strong>约束</strong>满足", "<strong>总额</strong>不变", "<strong>合法</strong>态", "<strong>保证</strong>"]}}, {"tag": "I/D", "name": "隔离与持久", "body": "<strong>隔离性</strong>：并发事务互不干扰；<strong>持久性</strong>：一旦提交，结果永久保存。", "points": {"title": "I/D", "items": ["<strong>隔离</strong>：并发", "<strong>持久</strong>：落盘", "<strong>提交</strong>后", "<strong>不丢</strong>"]}}];
let stepIdx=0;
const ACIDX={t:0};
function sceneInit(){ACIDX.t=0;}
function sceneReset(){ACIDX.t=0;}
function sceneUpdate(dt){ACIDX.t+=dt*speed;if(ACIDX.t>8)ACIDX.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 const s=Math.floor(ACIDX.t/2)%4;
 const names=['原子性 A：全做或全不做(回滚)','一致性 C：约束始终满足','隔离性 I：并发互不干扰','持久性 D：提交后永久'];
 drawText('事务 ACID：'+names[s],W/2,20,'#74b9ff',12);
 const cx=W/2,cy=H/2;
 if(s===0){ctx.fillStyle='#1e3a5f';ctx.fillRect(cx-50,cy-20,100,40);ctx.strokeStyle='#74b9ff';ctx.strokeRect(cx-50,cy-20,100,40);drawText('commit / rollback',cx,cy,'#fbbf24',10);}
 else if(s===1){ctx.strokeStyle='#34d399';ctx.strokeRect(cx-60,cy-30,120,60);drawText('约束有效',cx,cy,'#34d399',11);}
 else if(s===2){ctx.fillStyle='#1e3a5f';ctx.fillRect(cx-60,cy-30,50,60);ctx.fillStyle='#3a1e2a';ctx.fillRect(cx+10,cy-30,50,60);drawText('隔离',cx,cy,'#fbf4d0',11);}
 else {ctx.fillStyle='#9ab0d0';ctx.fillRect(cx-50,cy-10,100,20);drawText('已落盘',cx,cy,'#06281c',10);}
 drawText('ACID 保证数据库可靠：转账要么成功要么回滚',W/2,H-22,'#9ab0d0',11);}
