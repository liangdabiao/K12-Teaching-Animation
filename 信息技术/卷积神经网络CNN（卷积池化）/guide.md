...
## 1. 需求
动画展示 CNN 卷积核滑动提取特征与池化降维过程。

## 2. 专业思考
CNN 专为图像设计。动画左图+滑动卷积核提取特征、右特征图+池化降维。强调局部感受野、权值共享(同核复用、参少)、池化降维增强平移不变性、层级提取从边缘到语义。
...
<header class="topbar">
  ...
  <span class="title">CNN · 卷积神经网络</span>
  <span class="subtitle">卷积 + 池化</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>卷积</h4><div class="v">滑动提取特征</div></div><div class="card s3"><div class="num">2</div><h4>池化</h4><div class="v">降维保特征</div></div><div class="card s4"><div class="num">3</div><h4>权共</h4><div class="v">权值共享</div></div><div class="card "><div class="num">4</div><h4>应用</h4><div class="v">图像识别</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">CNN · 卷积神经网络</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察特征提取</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "CNN 用途", "body": "<strong>卷积神经网络(CNN)</strong>专为<strong>图像</strong>等网格数据设计，擅长特征提取与识别。", "points": {"title": "用途", "items": ["<strong>图像</strong>", "<strong>识别</strong>", "<strong>视觉</strong>", "<strong>CV</strong>核心"]}}, {"tag": "卷积", "name": "卷积层", "body": "<strong>卷积核</strong>在图上滑动，与局部区域做点积，提取边缘、纹理等特征。", "points": {"title": "卷积", "items": ["<strong>核</strong>滑动", "<strong>局部</strong>感受", "<strong>特征图</strong>", "<strong>边缘</strong>纹理"]}, "code": "特征图 = 输入 ⊛ 卷积核"}, {"tag": "池化", "name": "池化层", "body": "<strong>池化</strong>(如最大池化)对特征图下采样，<strong>降维</strong>并保留主要特征、增强平移不变。", "points": {"title": "池化", "items": ["<strong>最大</strong>池化", "<strong>降维</strong>", "<strong>不变</strong>性", "<strong>抗扰</strong>"]}}, {"tag": "权共", "name": "权值共享", "body": "同一卷积核在整图复用(<strong>权值共享</strong>)，大幅减少参数量、提升效率。", "points": {"title": "权共", "items": ["<strong>共享</strong>核", "<strong>参少</strong>", "<strong>高效</strong>", "<strong>平移</strong>不变"]}}];
let stepIdx=0;
const CNNX={t:0};
function sceneInit(){CNNX.t=0;}
function sceneReset(){CNNX.t=0;}
function sceneUpdate(dt){CNNX.t+=dt*speed;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('CNN：卷积核滑动提取特征 → 池化降维',W/2,20,'#74b9ff',12);
 const cx=W*0.25,cy=H/2;let x=cx+Math.sin(CNNX.t*2)*6;
 ctx.fillStyle='#1e3a5f';ctx.fillRect(cx-30,cy-30,60,60);ctx.strokeStyle='#74b9ff';ctx.strokeRect(cx-30,cy-30,60,60);
 ctx.fillStyle='#fbbf24';ctx.fillRect(x-12,cy-12,24,24);drawText('卷积核',x,cy-34,'#fbbf24',10);
 const px=W*0.7;ctx.fillStyle='#34d399';for(let i=0;i<4;i++)for(let j=0;j<4;j++){ctx.globalAlpha=0.3+0.4*((i+j)%2);ctx.fillRect(px-40+i*20,cy-40+j*20,18,18);}ctx.globalAlpha=1;
 drawText('特征图→池化',px,cy-58,'#34d399',10);
 drawText('局部感受野+权值共享→识别平移不变',W/2,H-22,'#9ab0d0',11);}
