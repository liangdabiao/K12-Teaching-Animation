...
## 1. 需求
动画展示 Transformer 自注意力机制中词与词之间的关注权重。

## 2. 专业思考
Transformer 自注意力。动画四个词，依次高亮一个‘焦点词’，向其他词连出强度不同的注意力线，表示它关注全句。强调 Q·Kᵀ/√d 算相似度、softmax 归一、对 V 加权求和得新表示；自注意力是 GPT/BERT 等大模型核心，擅长长依赖、可并行。
...
<header class="topbar">
  ...
  <span class="title">Transformer · 自注意力</span>
  <span class="subtitle">Q·Kᵀ/√d → softmax</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>自注意</h4><div class="v">词关注所有词</div></div><div class="card s3"><div class="num">2</div><h4>QKV</h4><div class="v">查询/键/值</div></div><div class="card s4"><div class="num">3</div><h4>softmax</h4><div class="v">权重归一</div></div><div class="card "><div class="num">4</div><h4>应用</h4><div class="v">NLP大模型</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">Transformer · 自注意力</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察注意力</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "自注意力", "body": "<strong>自注意力</strong>让序列中每个词都去<strong>关注(加权)所有词</strong>，捕捉长距离依赖。", "points": {"title": "机制", "items": ["<strong>每词</strong>看全句", "<strong>加权</strong>", "<strong>长依赖</strong>", "<strong>并行</strong>"]}}, {"tag": "QKV", "name": "Q/K/V", "body": "每个词生成<strong>查询 Q、键 K、值 V</strong>；用 Q 与各方 K 算相似度得到注意力权重。", "points": {"title": "QKV", "items": ["<strong>Q</strong>查询", "<strong>K</strong>键", "<strong>V</strong>值", "<strong>相似度</strong>"]}, "code": "score = Q·Kᵀ / √d"}, {"tag": "softmax", "name": "归一化", "body": "相似度经 <strong>softmax 归一</strong>得权重，对 V 加权求和得到该词的新表示。", "points": {"title": "softmax", "items": ["<strong>QKᵀ/√d</strong>", "<strong>softmax</strong>", "<strong>加权</strong>V", "<strong>新表示</strong>"]}, "code": "attn = softmax(QKᵀ/√d) · V"}, {"tag": "应用", "name": "应用", "body": "自注意力是 <strong>Transformer、GPT/BERT 等大模型</strong>的核心，推动 NLP 飞跃。", "points": {"title": "应用", "items": ["<strong>Transformer</strong>", "<strong>GPT</strong>", "<strong>BERT</strong>", "<strong>翻译</strong>生成"]}}];
let stepIdx=0;
const TRF={t:0,words:['猫','坐','垫','上'],Q:[],K:[]};
function sceneInit(){TRF.t=0;}
function sceneReset(){TRF.t=0;}
function sceneUpdate(dt){TRF.t+=dt*speed;if(TRF.t>4)TRF.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('Transformer 自注意力：每个词关注所有词',W/2,20,'#74b9ff',12);
 const n=TRF.words.length;const y=H/2;const xs=[];for(let i=0;i<n;i++){xs.push(W*0.2+i*(W*0.6/(n-1)));}
 const focus=Math.floor(TRF.t/n)%n;
 for(let i=0;i<n;i++){ctx.fillStyle=i===focus?'#34d399':'#1e3a5f';ctx.beginPath();ctx.arc(xs[i],y,20,0,7);ctx.fill();ctx.strokeStyle='#3b82f6';ctx.stroke();drawText(TRF.words[i],xs[i],y,'#fbf4d0',12);
  if(i!==focus){const a=0.2+0.6*Math.cos((i-focus)*1.2);ctx.strokeStyle='rgba(116,185,255,'+Math.max(0.1,a)+')';ctx.lineWidth=1+2*Math.max(0,a);ctx.beginPath();ctx.moveTo(xs[focus],y);ctx.lineTo(xs[i],y);ctx.stroke();}}
 drawText('Q·Kᵀ/√d → softmax → 加权和(V)：关注"坐"→"垫"',W/2,H-22,'#9ab0d0',11);}
