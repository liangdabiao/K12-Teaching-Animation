...
## 1. 需求
动画展示 Floyd 算法以 k 为中间点逐轮松弛距离矩阵。

## 2. 专业思考
Floyd 全源最短路，DP 思想。动画距离矩阵随中间点 k 逐轮松弛，高亮第 k 行/列。强调状态 d[i][j]=min(d[i][j], d[i][k]+d[k][j])、O(n³)、允许负权(无负环)、适合稠密小图。区别于 Dijkstra(单源)。
...
<header class="topbar">
  ...
  <span class="title">Floyd 算法 · 全源最短路</span>
  <span class="subtitle">O(n³) 插点松弛</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>目标</h4><div class="v">所有点对最短路</div></div><div class="card s3"><div class="num">2</div><h4>思想</h4><div class="v">以 k 为中转</div></div><div class="card s4"><div class="num">3</div><h4>松弛</h4><div class="v">d[i][j]>d[i][k]+d[k][j]</div></div><div class="card "><div class="num">4</div><h4>复杂</h4><div class="v">O(n³)</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">Floyd 算法 · 全源最短路</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察矩阵松弛</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "全源最短路", "body": "<strong>Floyd 算法</strong>求图中<strong>任意两点间最短路径</strong>，结果存于距离矩阵。", "points": {"title": "目标", "items": ["<strong>所有对</strong>", "<strong>距离矩阵</strong>", "<strong>DP</strong>", "<strong>经典</strong>"]}}, {"tag": "思想", "name": "插点思想", "body": "依次允许以每个顶点 k 作<strong>中转</strong>，尝试更新 i→j 的距离。", "points": {"title": "思想", "items": ["<strong>k</strong>：中转点", "<strong>依次</strong>放开", "<strong>更新</strong>矩阵", "<strong>DP</strong>"]}, "code": "允许经 k：更新所有 i,j"}, {"tag": "松弛", "name": "松弛公式", "body": "若 <strong>d[i][k] + d[k][j] < d[i][j]</strong>，则更新 d[i][j]。三重循环实现。", "points": {"title": "松弛", "items": ["<strong>三角</strong>不等式", "<strong>更优</strong>则更新", "<strong>三重</strong>循环", "<strong>O(n³)</strong>"]}, "code": "d[i][j]=min(d[i][j], d[i][k]+d[k][j])"}, {"tag": "应用", "name": "应用", "body": "适用于<strong>稠密图、点数不多</strong>的全源最短路；负权边也可(无负环)。", "points": {"title": "用途", "items": ["<strong>稠密</strong>图", "<strong>全源</strong>", "<strong>负权</strong>：可", "<strong>中转</strong>"]}}];
let stepIdx=0;
const FLOYD={t:0,n:4,dist:[],k:0};
function initD(){FLOYD.dist=[[0,3,8,Math.Infinity],[Math.Infinity,0,Math.Infinity,2],[Math.Infinity,Math.Infinity,0,1],[Math.Infinity,Math.Infinity,Math.Infinity,0]];}
function sceneInit(){initD();FLOYD.t=0;FLOYD.k=0;}
function sceneReset(){initD();FLOYD.t=0;FLOYD.k=0;}
function sceneUpdate(dt){FLOYD.t+=dt*speed;if(FLOYD.t>4)FLOYD.t=0;FLOYD.k=Math.floor(FLOYD.t);let d=FLOYD.dist.map(r=>r.slice());for(let kk=0;kk<=FLOYD.k;kk++)for(let i=0;i<4;i++)for(let j=0;j<4;j++)if(d[i][kk]+d[kk][j]<d[i][j])d[i][j]=d[i][kk]+d[kk][j];FLOYD.dist=d;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('Floyd：以 k 为中间点，松弛所有 i→j（O(n³)）',W/2,20,'#74b9ff',12);
 const cell=Math.min(70,(W-100)/5);const x0=80,y0=70;
 for(let i=0;i<4;i++)for(let j=0;j<4;j++){const x=x0+(j+1)*cell,y=y0+(i+1)*cell;const v=FLOYD.dist[i][j];ctx.fillStyle=(i===FLOYD.k||j===FLOYD.k)?'#1e3a5f':'#0d1117';ctx.strokeStyle='#243a5e';ctx.fillRect(x,y,cell,cell);ctx.strokeRect(x,y,cell,cell);drawText(v===Math.Infinity?'∞':String(v),x+cell/2,y+cell/2,'#fbbf24',12);}
 drawText('中间点 k = '+FLOYD.k,W/2,H-22,'#9ab0d0',12);}
