...
## 1. 需求
动画对比 FCFS 与 RR 两种 CPU 调度的 Gantt 图。

## 2. 专业思考
CPU 调度算法。动画切换 FCFS(按到达顺序、无抢占、护航效应)与 RR(时间片轮转、响应快公平)的 Gantt 图。强调调度目标(吞吐/响应/公平)、RR 时间片大小权衡、优先级调度需防饥饿(老化提升)。
...
<header class="topbar">
  ...
  <span class="title">CPU 调度算法</span>
  <span class="subtitle">FCFS / RR / 优先级</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>FCFS</h4><div class="v">先到先服务</div></div><div class="card s3"><div class="num">2</div><h4>RR</h4><div class="v">时间片轮转</div></div><div class="card s4"><div class="num">3</div><h4>优先级</h4><div class="v">高者先得</div></div><div class="card "><div class="num">4</div><h4>目标</h4><div class="v">公平/高效</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">CPU 调度算法</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察 Gantt</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "调度目标", "body": "<strong>CPU 调度</strong>决定就绪队列中哪个进程获得 CPU，目标是<strong>公平与高效</strong>。", "points": {"title": "目标", "items": ["<strong>吞吐</strong>量", "<strong>响应</strong>时间", "<strong>公平</strong>", "<strong>CPU</strong>利用"]}}, {"tag": "FCFS", "name": "先来先服务", "body": "<strong>FCFS</strong>：按到达顺序服务，简单但<strong>长作业可能让短作业久等</strong>(护航效应)。", "points": {"title": "FCFS", "items": ["<strong>顺序</strong>", "<strong>简单</strong>", "<strong>护航</strong>：长拖短", "<strong>无抢占</strong>"]}}, {"tag": "RR", "name": "时间片轮转", "body": "<strong>RR</strong>：每个进程分配<strong>时间片</strong>，用完即排到队尾，响应快、公平。", "points": {"title": "RR", "items": ["<strong>时间片</strong>", "<strong>轮转</strong>", "<strong>响应快</strong>", "<strong>公平</strong>"]}}, {"tag": "优先级", "name": "优先级调度", "body": "<strong>优先级调度</strong>：高优先级先执行；需防<strong>饥饿</strong>(低优久等)，可动态提升。", "points": {"title": "优先级", "items": ["<strong>高先</strong>", "<strong>饥饿</strong>风险", "<strong>老化</strong>：提优", "<strong>抢占</strong>：可"]}}];
let stepIdx=0;
const CPU={t:0,p:['P1','P2','P3'],algo:0};
function sceneInit(){CPU.t=0;}
function sceneReset(){CPU.t=0;}
function sceneUpdate(dt){CPU.t+=dt*speed;if(CPU.t>9)CPU.t=0;CPU.algo=Math.floor(CPU.t/4.5)%2;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText(CPU.algo===0?'CPU调度：FCFS 先到先服务':'CPU调度：时间片轮转 RR',W/2,20,'#74b9ff',12);
 const y=H/2,gantt=W*0.1,w=(W*0.8)/9;
 const seq=CPU.algo===0?['P1','P1','P1','P2','P2','P3','P3','P3','P3']:['P1','P2','P3','P1','P2','P3','P1','P2','P3'];
 const cur=CPU.algo===0?Math.floor(CPU.t)%9:Math.floor(CPU.t/0.5)%9;
 for(let i=0;i<9;i++){ctx.fillStyle=seq[i]==='P1'?'#34d399':seq[i]==='P2'?'#38bdf8':'#fbbf24';ctx.fillRect(gantt+i*w,y-20,w-3,40);drawText(seq[i],gantt+i*w+w/2-3,y,'#06281c',11);}
 ctx.strokeStyle='#f87171';ctx.lineWidth=2;ctx.strokeRect(gantt+cur*w,y-20,w-3,40);
 drawText('就绪队列按策略占用 CPU；调度影响等待/响应',W/2,H-22,'#9ab0d0',11);}
