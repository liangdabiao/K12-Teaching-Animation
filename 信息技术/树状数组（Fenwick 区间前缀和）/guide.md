...
## 1. 需求
动画展示树状数组的建树、前缀和查询与节点链接(lowbit)。

## 2. 专业思考
树状数组(Fenwick)用 lowbit 链接节点维护前缀和。动画画出 8 个元素对应的树状数组节点与父子连线，实时显示 sum(1..5)。强调查询 i-=lowbit(i)、更新 i+=lowbit(i) 均 O(log n)，适合动态单点修改+区间求和，常数小于线段树。
...
<header class="topbar">
  ...
  <span class="title">树状数组 · Fenwick</span>
  <span class="subtitle">O(log n) 前缀和</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>结构</h4><div class="v">lowbit 链接</div></div><div class="card s3"><div class="num">2</div><h4>前缀</h4><div class="v">sum(1..i)</div></div><div class="card s4"><div class="num">3</div><h4>更新</h4><div class="v">单点 O(log n)</div></div><div class="card "><div class="num">4</div><h4>应用</h4><div class="v">动态区间和</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">树状数组 · Fenwick</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察查询</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "树状数组", "body": "<strong>树状数组(Fenwick Tree)</strong>用数组以 lowbit 链接节点，高效维护<strong>前缀和</strong>与单点更新。", "points": {"title": "结构", "items": ["<strong>lowbit</strong>：链接", "<strong>数组</strong>存区间和", "<strong>高效</strong>", "<strong>简洁</strong>"]}, "code": "lowbit(x)=x&-x"}, {"tag": "前缀", "name": "前缀和查询", "body": "查询前缀和 sum(1..i)：从 i 沿 lowbit 不断减，累加所经节点值，<strong>O(log n)</strong>。", "points": {"title": "查询", "items": ["<strong>i-=lowbit</strong>", "<strong>累加</strong>节点", "<strong>O(log n)</strong>", "<strong>前缀</strong>"]}, "code": "while(i>0): s+=c[i]; i-=lowbit(i)"}, {"tag": "更新", "name": "单点更新", "body": "单点加值：从 i 沿 lowbit 不断加，更新所经节点，<strong>O(log n)</strong>。", "points": {"title": "更新", "items": ["<strong>i+=lowbit</strong>", "<strong>改节点</strong>", "<strong>O(log n)</strong>", "<strong>动态</strong>"]}, "code": "while(i<=n): c[i]+=v; i+=lowbit(i)"}, {"tag": "应用", "name": "应用", "body": "适用于<strong>频繁单点修改+区间求和</strong>的场景，比线段树常数更小。", "points": {"title": "用途", "items": ["<strong>动态</strong>区间和", "<strong>逆序对</strong>", "<strong>轻量</strong>", "<strong>竞赛</strong>常用"]}}];
let stepIdx=0;
const FT={t:0,arr:[3,2,5,1,4,6,2,3],tree:[],q:5};
function buildFT(){FT.tree=new Array(9).fill(0);for(let i=1;i<=8;i++){let idx=i;while(idx<=8){FT.tree[idx]+=FT.arr[i-1];idx+=idx&-idx;}}}
function sceneInit(){buildFT();FT.t=0;}
function sceneReset(){buildFT();FT.t=0;}
function sceneUpdate(dt){FT.t+=dt*speed;if(FT.t>6)FT.t=0;}
function prefix(n){let s=0;while(n>0){s+=FT.tree[n];n-=n&-n;}return s;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('树状数组 Fenwick：前缀和 query/单点更新 O(log n)',W/2,20,'#74b9ff',12);
 for(let i=1;i<=8;i++){const x=W*(i-0.5)/9,y=H/2;ctx.fillStyle='#1e3a5f';ctx.strokeStyle='#3b82f6';ctx.beginPath();ctx.arc(x,y,18,0,7);ctx.fill();ctx.stroke();drawText(FT.tree[i],x,y,'#fbbf24',11);
  const p=i-(i&-i);if(p>0){const px=W*(p-0.5)/9;ctx.strokeStyle='#243a5e';ctx.beginPath();ctx.moveTo(x,y+18);ctx.lineTo(px,y-18);ctx.stroke();}}
 const ay=H-40,cw=(W-40)/8;for(let i=0;i<8;i++){ctx.fillStyle='#1e3a5f';ctx.strokeStyle='#3b82f6';ctx.fillRect(20+i*cw,ay,cw-4,22);ctx.strokeRect(20+i*cw,ay,cw-4,22);drawText(FT.arr[i],20+i*cw+cw/2-2,ay+11,'#fbbf24',11);}
 drawText('sum(1..'+FT.q+') = '+prefix(FT.q),W/2,H-12,'#9ab0d0',12);}
