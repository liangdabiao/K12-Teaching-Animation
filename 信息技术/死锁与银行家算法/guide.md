...
## 1. 需求
动画展示银行家算法通过寻找安全序列判断资源分配是否安全。

## 2. 专业思考
死锁与银行家算法。动画列出各进程 Need 与系统 Available，实时计算并展示安全序列(存在则不会死锁)。强调死锁四必要条件、银行家‘分配前试探、有安全序才分’的避免策略、区别于死锁预防(破坏条件)。
...
<header class="topbar">
  ...
  <span class="title">死锁与银行家算法</span>
  <span class="subtitle">资源分配安全性</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>死锁</h4><div class="v">互相等待</div></div><div class="card s3"><div class="num">2</div><h4>条件</h4><div class="v">四必要条件</div></div><div class="card s4"><div class="num">3</div><h4>银行家</h4><div class="v">试探分配</div></div><div class="card "><div class="num">4</div><h4>安全</h4><div class="v">存在安全序列</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">死锁与银行家算法</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察安全序列</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "什么是死锁", "body": "<strong>死锁</strong>：多个进程互相等待对方持有的资源，均无法推进。", "points": {"title": "定义", "items": ["<strong>互等</strong>资源", "<strong>僵持</strong>", "<strong>多进程</strong>", "<strong>无解</strong>"]}}, {"tag": "条件", "name": "四必要条件", "body": "死锁需同时满足：<strong>互斥、占有且等待、不可剥夺、循环等待</strong>。", "points": {"title": "条件", "items": ["<strong>互斥</strong>", "<strong>占有等待</strong>", "<strong>不可剥夺</strong>", "<strong>循环等待</strong>"]}}, {"tag": "银行家", "name": "银行家算法", "body": "<strong>银行家算法</strong>：分配前试探，若仍存在<strong>安全序列</strong>(所有进程能按顺序完成)才真正分配，否则等待。", "points": {"title": "策略", "items": ["<strong>试探</strong>分配", "<strong>找安全序</strong>", "<strong>有则</strong>分", "<strong>无则</strong>等"]}, "code": "分配前：检查是否存在安全序列"}, {"tag": "安全", "name": "安全状态", "body": "处于<strong>安全状态</strong>则系统必能避免死锁；银行家以此保证安全性。", "points": {"title": "安全", "items": ["<strong>安全序</strong>存在", "<strong>可避免</strong>", "<strong>否则</strong>：不安全", "<strong>预防</strong>：破条件"]}}];
let stepIdx=0;
const BANK={t:0,avail:[3,3,2],max:[[7,5,3],[3,2,2],[9,0,2],[2,2,2],[4,3,3]],alloc:[[0,1,0],[2,0,0],[3,0,2],[2,1,1],[0,0,2]],need:[]};
function compNeed(){BANK.need=BANK.max.map((r,i)=>r.map((v,j)=>v-BANK.alloc[i][j]));}
function sceneInit(){compNeed();BANK.t=0;}
function sceneReset(){compNeed();BANK.t=0;}
function sceneUpdate(dt){BANK.t+=dt*speed;if(BANK.t>6)BANK.t=0;}
function safeSeq(){let work=BANK.avail.slice();let fin=BANK.alloc.map(()=>false);const seq=[];let changed=true;while(changed){changed=false;for(let i=0;i<5;i++){if(!fin[i]&&BANK.need[i].every((v,j)=>v<=work[j])){work=work.map((w,j)=>w+BANK.alloc[i][j]);fin[i]=true;seq.push(i);changed=true;}}}return fin.every(x=>x)?seq:null;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('银行家算法：试探分配，存在安全序列则不死锁',W/2,18,'#74b9ff',12);
 const seq=safeSeq();drawText('安全序列：'+(seq?seq.map(x=>'P'+x).join(' → '):'无(不安全)'),W/2,42,'#34d399',12);
 for(let i=0;i<5;i++){const y=70+i*24;ctx.fillStyle='#1e3a5f';ctx.fillRect(40,y,40,18);drawText('P'+i,60,y+9,'#fbbf24',10);ctx.fillStyle='#9ab0d0';drawText('Need:'+BANK.need[i].join(','),100,y+9,'#9ab0d0',10);}
 drawText('Available: '+BANK.avail.join(','),W/2,H-18,'#9ab0d0',11);}
