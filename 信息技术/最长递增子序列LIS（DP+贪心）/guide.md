...
## 1. 需求
动画展示 LIS 的动态规划/贪心二分过程及当前最长长度。

## 2. 专业思考
LIS 经典 DP。动画逐元素处理，下方显示以各位置结尾的 dp 值，实时给出当前 LIS 长度(贪心+二分 tail 数组)。强调 dp[i]=1+max{dp[j]|a[j]<a[i]}、优化用维护最小结尾+二分降到 O(n log n)。区别于 LCS(编辑距离)。
...
<header class="topbar">
  ...
  <span class="title">最长递增子序列 LIS</span>
  <span class="subtitle">DP + 二分 O(n log n)</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>问题</h4><div class="v">最长严格递增</div></div><div class="card s3"><div class="num">2</div><h4>DP</h4><div class="v">O(n²) 递推</div></div><div class="card s4"><div class="num">3</div><h4>优化</h4><div class="v">贪心+二分</div></div><div class="card "><div class="num">4</div><h4>应用</h4><div class="v">偏序/排版</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">最长递增子序列 LIS</span></h2>
...
const STEPS=[{"tag": "基础", "name": "什么是 LIS", "body": "<strong>最长递增子序列(LIS)</strong>：在原序列中删去若干元素，使剩余严格递增且长度最大。", "points": {"title": "定义", "items": ["<strong>递增</strong>", "<strong>子序列</strong>：保序", "<strong>最长</strong>", "<strong>经典</strong>"]}}, {"tag": "DP", "name": "动态规划", "body": "DP：<strong>dp[i]=以第 i 个结尾的 LIS 长度</strong>，枚举之前更小的元素取最大+1，O(n²)。", "points": {"title": "DP", "items": ["<strong>dp[i]</strong>", "<strong>枚举</strong>j<i", "<strong>max+1</strong>", "<strong>O(n²)</strong>"]}, "code": "dp[i]=1+max{dp[j]}(a[j]<a[i])"}, {"tag": "优化", "name": "贪心+二分", "body": "维护‘长度为 k 的最小结尾’，新元素用<strong>二分</strong>替换，复杂度降到 <strong>O(n log n)</strong>。", "points": {"title": "优化", "items": ["<strong>tail</strong>数组", "<strong>二分</strong>定位", "<strong>O(n log n)</strong>", "<strong>更优</strong>"]}, "code": "for x: 二分找位置替换 tail"}, {"tag": "应用", "name": "应用", "body": "LIS 用于<strong>偏序集最长链、导弹拦截、排版换行</strong>等问题。", "points": {"title": "用途", "items": ["<strong>偏序</strong>", "<strong>拦截</strong>", "<strong>排版</strong>", "<strong>子序列</strong>"]}}];
let stepIdx=0;
const LIS={t:0,A:[3,1,4,1,5,9,2,6]};
function sceneInit(){LIS.t=0;}
function sceneReset(){LIS.t=0;}
function sceneUpdate(dt){LIS.t+=dt*speed;if(LIS.t>LIS.A.length)LIS.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('最长递增子序列 LIS（贪心+二分，O(n log n)）',W/2,20,'#74b9ff',12);
 const n=LIS.A.length;const cw=(W-60)/n;const filled=Math.floor(LIS.t);
 const dp=[];let tail=[];for(let i=0;i<n;i++){const x=LIS.A[i];let l=0,r=tail.length;while(l<r){const m=(l+r)>>1;if(tail[m]>=x)r=m;else l=m+1;}if(l===tail.length)tail.push(x);else tail[l]=x;dp[i]=l+1;}
 for(let i=0;i<n;i++){const x=30+i*cw,y=H/2;const inSeq=i<filled;const active=i===filled-1;ctx.fillStyle=active?'#34d399':(inSeq?'#1e3a5f':'#0d1117');ctx.strokeStyle='#3b82f6';ctx.fillRect(x,y-20,cw-6,40);ctx.strokeRect(x,y-20,cw-6,40);drawText(LIS.A[i],x+cw/2-3,y,'#fbbf24',12);}
 drawText('dp(以i结尾LIS长)：'+dp.slice(0,filled).join(','),W/2,H-26,'#9ab0d0',11);
 drawText('当前 LIS 长度 = '+tail.length,W/2,H-10,'#34d399',12);}
