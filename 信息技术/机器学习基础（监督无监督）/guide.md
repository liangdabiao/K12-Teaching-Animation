...
## 1. 需求
动画对比监督学习(带标签拟合决策边界)与无监督学习(聚类)。

## 2. 专业思考
机器学习基础。动画两态：监督学习(带标签点+决策边界)、无监督学习(无标签点聚成两簇)。强调三要素(数据/模型/训练)、监督(分类/回归)与无监督(聚类/降维)区别、泛化能力的重要性。
...
<header class="topbar">
  ...
  <span class="title">机器学习基础</span>
  <span class="subtitle">监督学习 / 无监督学习</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>定义</h4><div class="v">从数据学映射</div></div><div class="card s3"><div class="num">2</div><h4>监督</h4><div class="v">带标签训练</div></div><div class="card s4"><div class="num">3</div><h4>无监督</h4><div class="v">无标签聚类</div></div><div class="card "><div class="num">4</div><h4>要素</h4><div class="v">数据/模型/训练</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">机器学习基础</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察两类</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "什么是 ML", "body": "<strong>机器学习</strong>让程序从数据中自动学习规律(映射 f)，而非显式编程。", "points": {"title": "概念", "items": ["<strong>数据</strong>驱动", "<strong>学映射</strong>", "<strong>非编程</strong>", "<strong>预测</strong>"]}}, {"tag": "监督", "name": "监督学习", "body": "<strong>监督学习</strong>用<strong>带标签</strong>数据训练，学习输入到输出的映射(分类/回归)。", "points": {"title": "监督", "items": ["<strong>有标签</strong>", "<strong>分类</strong>回归", "<strong>训练</strong>", "<strong>泛化</strong>"]}, "code": "(x,y)标签 → 学 f(x)≈y"}, {"tag": "无监督", "name": "无监督学习", "body": "<strong>无监督学习</strong>面对<strong>无标签</strong>数据，自动发现结构(如聚类)。", "points": {"title": "无监督", "items": ["<strong>无标签</strong>", "<strong>聚类</strong>", "<strong>降维</strong>", "<strong>发现</strong>结构"]}, "code": "x → 聚成簇"}, {"tag": "要素", "name": "三要素", "body": "ML 三要素：<strong>数据、模型、算法(训练)</strong>；性能靠数据量与泛化能力。", "points": {"title": "要素", "items": ["<strong>数据</strong>", "<strong>模型</strong>", "<strong>训练</strong>", "<strong>泛化</strong>"]}}];
let stepIdx=0;
const ML={t:0,pts:[]};
function sceneInit(){ML.t=0;ML.pts=[];for(let i=0;i<10;i++){ML.pts.push({x:W0()*0.6+40,y:H2()+Math.random()*H3(),c:i%2});}}
function sceneReset(){ML.t=0;ML.pts=[];for(let i=0;i<10;i++){ML.pts.push({x:W0()*0.6+40,y:H2()+Math.random()*H3(),c:i%2});}}
function W0(){return 800;}function H2(){return 120;}function H3(){return 200;}
function sceneUpdate(dt){ML.t+=dt*speed;if(ML.t>6)ML.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 const s=Math.floor(ML.t/3)%2;
 drawText(s?'监督学习：带标签训练，拟合决策边界':'无监督学习：无标签，聚类发现结构',W/2,20,'#74b9ff',12);
 for(const p of ML.pts){ctx.fillStyle=p.c?'#34d399':'#f87171';ctx.beginPath();ctx.arc(p.x,p.y,5,0,7);ctx.fill();}
 if(s){ctx.strokeStyle='#74b9ff';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(W*0.2,H*0.7);ctx.lineTo(W*0.8,H*0.3);ctx.stroke();drawText('决策边界',W*0.7,H*0.3-10,'#74b9ff',11);}
 else{ctx.strokeStyle='#fbbf24';ctx.lineWidth=2;ctx.beginPath();ctx.arc(W*0.4,H/2,40,0,7);ctx.stroke();ctx.beginPath();ctx.arc(W*0.65,H/2+20,40,0,7);ctx.stroke();drawText('簇',W*0.4,H/2-50,'#fbbf24',11);}
 drawText('模型=从数据中学到的映射 f(x)≈y',W/2,H-22,'#9ab0d0',11);}
