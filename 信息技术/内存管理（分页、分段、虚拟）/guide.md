...
## 1. 需求
动画展示分页(页表映射)与分段(逻辑段)两种内存管理方式。

## 2. 专业思考
内存管理。动画两态：分页(虚拟页经页表映射到离散物理帧、无外部碎片)、分段(按代码/数据/堆/栈逻辑段、便于共享保护)。强调虚拟内存让进程用连续虚地址、物理可离散换出、扩大可用空间。
...
<header class="topbar">
  ...
  <span class="subtitle">分页 / 分段 / 虚拟内存</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>分页</h4><div class="v">固定页→帧</div></div><div class="card s3"><div class="num">2</div><h4>分段</h4><div class="v">按逻辑段</div></div><div class="card s4"><div class="num">3</div><h4>虚拟</h4><div class="v">连续虚地址</div></div><div class="card "><div class="num">4</div><h4>表</h4><div class="v">页表/段表</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">内存管理</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察地址转换</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "内存管理", "body": "<strong>内存管理</strong>负责分配回收内存、实现<strong>虚拟地址到物理地址</strong>的映射。", "points": {"title": "职责", "items": ["<strong>分配</strong>", "<strong>回收</strong>", "<strong>映射</strong>", "<strong>保护</strong>"]}}, {"tag": "分页", "name": "分页", "body": "<strong>分页</strong>把内存分为固定大小页/帧，用<strong>页表</strong>把虚拟页映射到物理帧（离散装入）。", "points": {"title": "分页", "items": ["<strong>页</strong>=<strong>帧</strong>", "<strong>页表</strong>映射", "<strong>离散</strong>装", "<strong>无外碎</strong>"]}, "code": "虚拟页 → 页表 → 物理帧"}, {"tag": "分段", "name": "分段", "body": "<strong>分段</strong>按逻辑意义(代码/数据/栈)划分变长段，用段表管理，便于共享保护。", "points": {"title": "分段", "items": ["<strong>逻辑段</strong>", "<strong>变长</strong>", "<strong>段表</strong>", "<strong>共享</strong>便"]}}, {"tag": "虚拟", "name": "虚拟内存", "body": "<strong>虚拟内存</strong>让进程用连续虚拟地址，物理上可离散、部分换出磁盘，扩大可用空间。", "points": {"title": "虚拟", "items": ["<strong>连续虚址</strong>", "<strong>离散物</strong>", "<strong>换入换出</strong>", "<strong>扩空间</strong>"]}}];
let stepIdx=0;
const MEM={t:0};
function sceneInit(){MEM.t=0;}
function sceneReset(){MEM.t=0;}
function sceneUpdate(dt){MEM.t+=dt*speed;if(MEM.t>6)MEM.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 const s=Math.floor(MEM.t/3)%2;
 drawText(s?'分页：虚拟页→页表→物理帧(离散装入)':'分段：按逻辑段(代码/数据/栈)分配',W/2,20,'#74b9ff',12);
 if(s){const vx=W*0.2,px=W*0.65;for(let i=0;i<4;i++){ctx.fillStyle='#1e3a5f';ctx.fillRect(vx,80+i*40,40,32);drawText('v'+i,vx+20,96+i*40,'#fbbf24',10);ctx.fillStyle='#34d399';ctx.fillRect(px,80+i*40,40,32);drawText('f'+((i*2)%8),px+20,96+i*40,'#06281c',10);}ctx.strokeStyle='#9ab0d0';ctx.beginPath();ctx.moveTo(vx+40,96);ctx.lineTo(px,96);ctx.stroke();drawText('页表',(vx+px)/2,70,'#9ab0d0',11);}
 else{const segs=['代码','数据','堆','栈'];for(let i=0;i<4;i++){ctx.fillStyle=['#34d399','#38bdf8','#fbbf24','#f87171'][i];ctx.fillRect(W*0.2,80+i*40,200,32);drawText(segs[i],W*0.2+100,96+i*40,'#06281c',10);}}
 drawText('虚拟内存：进程用连续虚拟地址，物理离散',W/2,H-22,'#9ab0d0',11);}
