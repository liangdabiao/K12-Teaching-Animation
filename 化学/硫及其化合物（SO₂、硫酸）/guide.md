...
## 1. 需求
动画展示硫、二氧化硫、硫酸三种含硫物质及其转化路线。

## 2. 专业思考
硫及化合物。动画三屏：硫单质(黄、燃蓝焰)、SO₂(刺激、酸雨)、硫酸(强酸、工业之母)，底部给出接触法路线 S→SO₂→SO₃→H₂SO₄。强调 SO₂ 是酸雨主因、浓硫酸特性的特殊性(吸水/脱水/强氧化)。
...
<header class="topbar">
  ...
  <span class="title">硫及其化合物</span>
  <span class="subtitle">S→SO₂→H₂SO₄</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>硫</h4><div class="v">黄色固体</div></div><div class="card s3"><div class="num">2</div><h4>SO₂</h4><div class="v">刺激性·酸雨</div></div><div class="card s4"><div class="num">3</div><h4>硫酸</h4><div class="v">强酸·工业</div></div><div class="card "><div class="num">4</div><h4>路线</h4><div class="v">接触法制酸</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">硫及其化合物</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放切换化合物</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "硫单质", "body": "硫(S)为<strong>黄色固体</strong>，燃烧呈<strong>淡蓝色火焰</strong>，生成有刺激性气味的 SO₂。", "points": {"title": "性质", "items": ["<strong>黄色</strong>固体", "<strong>燃蓝焰</strong>", "<strong>S+O₂</strong>→SO₂", "<strong>嗅</strong>：刺激"]}}, {"tag": "SO2", "name": "二氧化硫", "body": "SO₂ 有<strong>刺激性气味</strong>，溶于水生成亚硫酸，是<strong>酸雨</strong>的主要元凶之一。", "points": {"title": "SO₂", "items": ["<strong>刺激</strong>味", "<strong>溶于水</strong>：亚硫酸", "<strong>漂白</strong>：品红", "<strong>酸雨</strong>：元凶"]}}, {"tag": "硫酸", "name": "硫酸", "body": "硫酸(H₂SO₄)是<strong>强酸、高沸点</strong>，具强腐蚀性、吸水性、脱水性，称‘工业之母’。", "points": {"title": "H₂SO₄", "items": ["<strong>强酸</strong>", "<strong>吸水</strong>脱水", "<strong>腐蚀</strong>", "<strong>用途</strong>：广"]}}, {"tag": "路线", "name": "接触法制酸", "body": "工业上 <strong>S→SO₂→SO₃→H₂SO₄</strong>（接触法）。", "points": {"title": "路线", "items": ["<strong>燃烧</strong>：S→SO₂", "<strong>催化</strong>：SO₂→SO₃", "<strong>吸收</strong>：→H₂SO₄", "<strong>接触法</strong>"]}}];
let stepIdx=0;
const SULF={t:0};
function sceneInit(){SULF.t=0;}
function sceneReset(){SULF.t=0;}
function sceneUpdate(dt){SULF.t+=dt*speed;if(SULF.t>6)SULF.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 const s=Math.floor(SULF.t/2)%3;
 const names=['S 硫：黄色固体，燃蓝焰','SO₂：刺激性，酸雨元凶','H₂SO₄：强酸，工业之母'];
 drawText('硫及其化合物：'+names[s],W/2,22,'#74b9ff',13);
 const cx=W/2,cy=H/2;
 if(s===0){ctx.fillStyle='#fbbf24';for(let i=0;i<8;i++){ctx.beginPath();ctx.arc(cx-40+i*12,cy+Math.sin(i)*10,7,0,7);ctx.fill();}drawText('S₈ 环状分子',cx,cy+50,'#fbbf24',12);}
 else if(s===1){ctx.fillStyle='#9ab0d0';ctx.beginPath();ctx.arc(cx,cy,26,0,7);ctx.fill();drawText('SO₂',cx,cy,'#0a1628',12);drawText('溶于水→亚硫酸→酸雨',cx,cy+50,'#9ab0d0',12);}
 else {ctx.fillStyle='#1e3a5f';roundRect(cx-40,cy-20,80,40,6);ctx.fill();ctx.strokeStyle='#74b9ff';ctx.stroke();drawText('H₂SO₄',cx,cy,'#fbbf24',12);drawText('浓硫酸：强腐蚀、吸水性',cx,cy+50,'#9ab0d0',12);}
 drawText('S → SO₂ → SO₃ → H₂SO₄（接触法制硫酸）',W/2,H-22,'#9ab0d0',11);}
