...
## 1. 需求
动画对比 C₄H₁₀（正丁烷/异丁烷）与 C₂H₆O（乙醇/二甲醚），说明同分异构体“分子式同、结构异”。

## 2. 专业思考
同分异构是有机化学难点。动画用两屏：第一屏 C₄H₁₀ 的碳链异构（直链 vs 支链），第二屏 C₂H₆O 的官能团异构（醇 –OH vs 醚 C–O–C）。强调“结构决定性质”：即便原子组成相同，连接方式不同（碳骨架、官能团位置/种类）导致物理化学性质差异。可提示异构体数目随碳数激增，是有机物种类繁多的主因。
...
<header class="topbar">
  ...
  <span class="title">同分异构体 · 分子式同结构异</span>
  <span class="subtitle">C₄H₁₀ / C₂H₆O</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>定义</h4><div class="v">分子式同、结构不同</div></div><div class="card s3"><div class="num">2</div><h4>类别</h4><div class="v">碳链/位置/官能团异构</div></div><div class="card s4"><div class="num">3</div><h4>性质</h4><div class="v">结构不同→性质不同</div></div><div class="card "><div class="num">4</div><h4>示例</h4><div class="v">丁烷、乙醇/二甲醚</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">同分异构体 · 分子式同结构异</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放切换示例</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "什么是同分异构", "body": "<strong>同分异构体</strong>：分子式相同，但<strong>原子连接顺序或空间排布不同</strong>，因而结构不同的化合物。", "points": {"title": "关键", "items": ["<strong>分子式</strong>：相同", "<strong>结构</strong>：不同", "<strong>类别</strong>：碳链/位置/官能团", "<strong>结果</strong>：性质不同"]}, "code": "同分异构 = 同分子式 + 不同结构"}, {"tag": "碳链", "name": "碳链异构", "body": "以 <strong>C₄H₁₀</strong> 为例：直链的<strong>正丁烷</strong>与带支链的<strong>异丁烷</strong>互为碳链异构体。", "points": {"title": "C₄H₁₀", "items": ["<strong>正丁烷</strong>：直链 4 碳", "<strong>异丁烷</strong>：支链", "<strong>沸点</strong>：正>异", "<strong>数量</strong>：2 种"]}}, {"tag": "官能团", "name": "官能团异构", "body": "以 <strong>C₂H₆O</strong> 为例：<strong>乙醇 CH₃CH₂OH</strong>（醇）与<strong>二甲醚 CH₃OCH₃</strong>（醚）官能团不同，性质迥异。", "points": {"title": "C₂H₆O", "items": ["<strong>乙醇</strong>：含 –OH", "<strong>二甲醚</strong>：含 C–O–C", "<strong>状态</strong>：乙醇液/醚气", "<strong>类别</strong>：醇 ≠ 醚"]}}, {"tag": "意义", "name": "结构与性质", "body": "同分异构说明<strong>结构决定性质</strong>：即便原子种类数目相同，连接方式不同也会导致沸点、反应性等差异。", "points": {"title": "启示", "items": ["<strong>连接顺序</strong>很关键", "有机物种数爆炸原因", "<strong>药物</strong>：异构体活性差", "<strong>分析</strong>：需辨结构"]}}];
let stepIdx=0;
const ISO={t:0,set:0};
function sceneInit(){ISO.t=0;}
function sceneReset(){ISO.t=0;}
function sceneUpdate(dt){ISO.t+=dt*speed;if(ISO.t>6)ISO.t=0;ISO.set=Math.floor(ISO.t/3)%2;}
function drawC(x,y,label){ctx.fillStyle='#1e3a5f';ctx.strokeStyle='#74b9ff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(x,y,16,0,7);ctx.fill();ctx.stroke();drawText(label,x,y,'#fbbf24',12);}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 if(ISO.set===0){
  drawText('同分异构：分子式 C₄H₁₀ 相同，碳骨架不同',W/2,22,'#74b9ff',14);
  drawText('正丁烷 n-butane',W*0.27,86,'#34d399',13);
  for(let i=0;i<4;i++){if(i>0){ctx.strokeStyle='#74b9ff';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(W*0.17+i*42,H/2);ctx.lineTo(W*0.17+(i+1)*42,H/2);ctx.stroke();}drawC(W*0.17+i*42,H/2,'C');}
  drawText('异丁烷 isobutane',W*0.76,86,'#fbbf24',13);
  const cx2=W*0.72,cy2=H/2;drawC(cx2,cy2,'C');
  [[-1,-1],[1,-1],[0,1]].forEach(d=>{const x=cx2+d[0]*52,y=cy2+d[1]*52;ctx.strokeStyle='#74b9ff';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(cx2,cy2);ctx.lineTo(x,y);ctx.stroke();drawC(x,y,'C');});
  drawText('连接方式不同 → 沸点/性质不同',W/2,H-26,'#9ab0d0',12);
 }else{
  drawText('同分异构：C₂H₆O → 乙醇 或 二甲醚',W/2,22,'#74b9ff',14);
  drawText('乙醇 CH₃CH₂OH（醇）',W*0.27,86,'#34d399',13);
  drawC(W*0.17,H/2,'C');drawC(W*0.17+52,H/2,'C');ctx.strokeStyle='#74b9ff';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(W*0.17,H/2);ctx.lineTo(W*0.17+52,H/2);ctx.stroke();drawText('–OH',W*0.17+52,H/2+36,'#f87171',12);
  drawText('二甲醚 CH₃OCH₃（醚）',W*0.76,86,'#fbbf24',13);
  drawC(W*0.66,H/2,'C');drawC(W*0.84,H/2,'C');const ox=(W*0.66+W*0.84)/2;ctx.strokeStyle='#74b9ff';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(W*0.66,H/2);ctx.lineTo(ox,H/2);ctx.lineTo(W*0.84,H/2);ctx.stroke();ctx.fillStyle='#f87171';ctx.beginPath();ctx.arc(ox,H/2,14,0,7);ctx.fill();drawText('O',ox,H/2,'#fff',12);
  drawText('官能团不同（–OH vs C–O–C）→ 类别与性质不同',W/2,H-26,'#9ab0d0',12);
 }}
