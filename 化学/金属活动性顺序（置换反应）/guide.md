...
## 1. 需求
动画展示金属活动性顺序，并用 Zn 放入 CuSO₄ 的置换反应演示“强换弱”的现象。

## 2. 专业思考
金属活动性是电化学与置换反应的核心。动画分两部分：上方顺序条（高亮 Zn），下方烧杯演示 Zn + CuSO₄ → ZnSO₄ + Cu：锌条溶解、红色铜析出、蓝色（Cu²⁺）变浅。强调“前换后、强换弱”，并点出氢前金属可置换酸中氢（如 Zn+2HCl→ZnCl₂+H₂↑）。可延伸湿法炼铜与牺牲阳极保护法。
...
<header class="topbar">
  ...
  <span class="title">金属活动性顺序 · 置换反应</span>
  <span class="subtitle">K Ca Na Mg Al Zn Fe Cu Ag Au</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>顺序</h4><div class="v">K Ca Na Mg Al Zn Fe Cu Ag Au</div></div><div class="card s3"><div class="num">2</div><h4>规律</h4><div class="v">前换后，强换弱</div></div><div class="card s4"><div class="num">3</div><h4>现象</h4><div class="v">活泼金属溶解、析出弱者</div></div><div class="card "><div class="num">4</div><h4>应用</h4><div class="v">湿法炼铜、防腐</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">金属活动性顺序 · 置换反应</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察 Zn 置换 Cu</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "活动性顺序", "body": "常见金属活动性由强到弱：<strong>K Ca Na Mg Al Zn Fe Cu Ag Au</strong>。越靠前越易失电子、越活泼。", "points": {"title": "记忆", "items": ["<strong>钾钙钠镁铝</strong> 很活泼", "<strong>锌铁</strong> 居中", "<strong>铜汞银铂金</strong> 不活泼", "氢前金属可置换酸中氢"]}, "code": "K Ca Na Mg Al Zn Fe | H | Cu Hg Ag Pt Au\n（左强右弱）"}, {"tag": "规律", "name": "强置换弱", "body": "<strong>排在前面的金属</strong>能把<strong>后面的金属</strong>从其盐溶液中置换出来（前换后、强换弱）。", "points": {"title": "规律", "items": ["<strong>Zn + CuSO₄</strong> 能反应", "<strong>Cu + ZnSO₄</strong> 不反应", "依据：活泼性 Zn>Cu", "氢前金属可换酸中 H"]}}, {"tag": "演示", "name": "Zn 置换 Cu", "body": "把<strong>锌条</strong>放入<strong>硫酸铜溶液</strong>：锌逐渐溶解，表面析出<strong>红色铜</strong>，蓝色溶液变浅。反应 Zn + CuSO₄ → ZnSO₄ + Cu。", "points": {"title": "现象", "items": ["<strong>锌条</strong>：变细溶解", "<strong>红色固体</strong>：析出 Cu", "<strong>蓝色</strong>：Cu²⁺减少变浅", "<strong>放热</strong>：轻微"]}}, {"tag": "应用", "name": "应用", "body": "利用该规律可<strong>湿法炼铜</strong>、判断金属防腐顺序、解释<strong>镀层/牺牲阳极</strong>保护法。", "points": {"title": "实例", "items": ["<strong>湿法炼铜</strong>：Fe+CuSO₄", "<strong>牺牲阳极</strong>：Zn 保船体", "<strong>判断</strong>：能否反应", "<strong>防腐</strong>：活泼金属先耗"]}}];
let stepIdx=0;
const MET={t:0,series:[['K',0],['Ca',1],['Na',2],['Mg',3],['Al',4],['Zn',5],['Fe',6],['Cu',7],['Ag',8],['Au',9]]};
function sceneInit(){MET.t=0;}
function sceneReset(){MET.t=0;}
function sceneUpdate(dt){MET.t+=dt*speed;if(MET.t>8)MET.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('金属活动性顺序：K Ca Na Mg Al Zn Fe Cu Ag Au（强 → 弱）',W/2,22,'#74b9ff',13);
 const cw=(W-120)/MET.series.length;
 MET.series.forEach((m,i)=>{const x=60+i*cw;ctx.fillStyle=m[0]==='Zn'?'#fbbf24':'#1e3a5f';ctx.strokeStyle='#3b82f6';ctx.lineWidth=1;ctx.fillRect(x,56,cw-6,40);ctx.strokeRect(x,56,cw-6,40);drawText(m[0],x+cw/2-3,76,'#fbf4d0',11);});
 const bx=W/2-90,by=H-60,bw=180,bh=180;
 const fade=Math.min(1,MET.t/4);
 ctx.fillStyle='rgba(56,189,248,'+(0.75-0.45*fade)+')';roundRect(bx,by-bh,bw,bh,8);ctx.fill();
 ctx.strokeStyle='#3b82f6';ctx.lineWidth=2;roundRect(bx,by-bh,bw,bh,8);ctx.stroke();
 const znH=90*(1-0.6*fade);ctx.fillStyle='#cbd5e1';ctx.fillRect(bx+30,by-20-znH,22,znH);
 ctx.fillStyle='#f87171';ctx.fillRect(bx+30,by-20-znH,22,28*fade);
 drawText('Zn + CuSO₄ → ZnSO₄ + Cu',W/2,by-bh-20,'#9ab0d0',12);
 drawText('活泼金属(Zn)从盐溶液置换出较不活泼金属(Cu)，蓝色逐渐变浅',W/2,by+24,'#9ab0d0',12);}
