...
## 1. 需求
动画展示盖斯定律：同一反应不同路径的反应热相等。

## 2. 专业思考
盖斯定律。动画状态图：H₂+½O₂(始态)经直接或分步两条路径到H₂O(终态)，下方说明ΔH相等、ΔH总=ΣΔH。强调反应热只取决于始终态、逆向变号、放大倍乘，可用于推算难测反应热。
...
<header class="topbar">
  ...
  <span class="title">盖斯定律 · 反应热加和</span>
  <span class="subtitle">ΔH 与路径无关</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>定律</h4><div class="v">ΔH 只与始终态</div></div><div class="card s3"><div class="num">2</div><h4>路径</h4><div class="v">可分多步</div></div><div class="card s4"><div class="num">3</div><h4>加和</h4><div class="v">ΔH总=ΣΔH</div></div><div class="card "><div class="num">4</div><h4>应用</h4><div class="v">求难测反应热</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">盖斯定律 · 反应热加和</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察两条路径</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "盖斯定律", "body": "<strong>盖斯定律</strong>：化学反应的反应热只与始态和终态有关，与反应途径无关。", "points": {"title": "定律", "items": ["<strong>ΔH</strong>：与路径无关", "<strong>始态</strong>终态", "<strong>关键</strong>", "<strong>热化学</strong>"]}, "code": "ΔH总 = Σ ΔH分步"}, {"tag": "路径", "name": "分步反应", "body": "一个反应可分成几步进行，每步反应热可不同，但总和不变。", "points": {"title": "分步", "items": ["<strong>可拆</strong>：多步", "<strong>各步</strong>：ΔH", "<strong>总</strong>：相同", "<strong>图</strong>：状态"]}}, {"tag": "加和", "name": "反应热加和", "body": "<strong>ΔH总 = ΔH₁ + ΔH₂ + …</strong>，用于计算难直接测的反应热。", "points": {"title": "计算", "items": ["<strong>加和</strong>：ΔH", "<strong>逆向</strong>：变号", "<strong>放大</strong>：倍乘", "<strong>应用</strong>：求热"]}}, {"tag": "应用", "name": "应用", "body": "利用已知反应热推算<strong>难以直接测量的反应热</strong>（如某些燃烧热）。", "points": {"title": "用途", "items": ["<strong>间接</strong>测热", "<strong>燃烧热</strong>", "<strong>生成热</strong>", "<strong>设计</strong>：工艺"]}}];
let stepIdx=0;
const HESS={t:0};
function sceneInit(){HESS.t=0;}
function sceneReset(){HESS.t=0;}
function sceneUpdate(dt){HESS.t+=dt*speed;if(HESS.t>6)HESS.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('盖斯定律：反应热只与始态终态有关，与路径无关',W/2,22,'#74b9ff',13);
 const cx=W/2,cy=H/2;
 ctx.fillStyle='#1e3a5f';ctx.strokeStyle='#74b9ff';ctx.beginPath();ctx.arc(cx,cy+50,24,0,7);ctx.fill();ctx.stroke();drawText('H₂+½O₂',cx,cy+50,'#fbbf24',11);
 ctx.fillStyle='#34d399';ctx.beginPath();ctx.arc(cx-100,cy-30,24,0,7);ctx.fill();ctx.stroke();drawText('H',cx-100,cy-30,'#06281c',11);
 drawText('直接 → H₂O  ΔH=-286 kJ/mol',cx-100,cy-70,'#34d399',12);
 drawText('分步 → H·→H₂O  ΔH 相等',cx+100,cy-70,'#fbbf24',12);
 ctx.strokeStyle='#9ab0d0';ctx.setLineDash([5,4]);ctx.beginPath();ctx.moveTo(cx-100,cy-30);ctx.lineTo(cx,cy+50);ctx.stroke();ctx.beginPath();ctx.moveTo(cx,cy+50);ctx.lineTo(cx+100,cy-30);ctx.stroke();ctx.setLineDash([]);
 ctx.fillStyle='#34d399';ctx.beginPath();ctx.arc(cx+100,cy-30,24,0,7);ctx.fill();ctx.stroke();drawText('H₂O',cx+100,cy-30,'#06281c',11);
 drawText('ΔH总 = ΔH₁ + ΔH₂（路径加和）',W/2,H-22,'#9ab0d0',12);}
