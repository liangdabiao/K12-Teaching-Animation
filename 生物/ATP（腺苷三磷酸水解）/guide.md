...
## 1. 需求
动画展示 ATP 的结构、水解放能与合成储能的可逆过程，说明其作为直接能源物质的角色。

## 2. 专业思考
ATP 是代谢核心。动画用腺苷块 + 三个磷酸球，时间过半时末端磷酸（绿色）脱离表示水解放能，否则表示由呼吸/光合储能合成。强调：ATP 是直接能源（≠储能物质如糖、脂肪）；水解与合成酶不同，故“不可逆”于酶水平但物质可逆。能量用于主动运输、肌肉收缩、合成等。
...
<header class="topbar">
  ...
  <span class="title">ATP · 细胞的能量通货</span>
  <span class="subtitle">ATP ⇌ ADP + Pi + 能量</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>结构</h4><div class="v">腺苷 + 3 磷酸</div></div><div class="card s3"><div class="num">2</div><h4>水解</h4><div class="v">末端磷酸键断裂放能</div></div><div class="card s4"><div class="num">3</div><h4>合成</h4><div class="v">呼吸/光合重新储能</div></div><div class="card "><div class="num">4</div><h4>角色</h4><div class="v">直接能源物质</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">ATP · 细胞的能量通货</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察水解/合成</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "ATP 结构", "body": "<strong>ATP（腺苷三磷酸）</strong>= 腺嘌呤 + 核糖 + 3 个磷酸基团。相邻磷酸间的高能键储存大量能量。", "points": {"title": "组成", "items": ["<strong>腺苷</strong>=腺嘌呤+核糖", "<strong>3×P</strong>：磷酸基团", "<strong>高能磷酸键</strong>：~", "<strong>符号</strong>：A–P~P~P"]}, "code": "ATP = A—P~P~P\n~ 表示高能磷酸键"}, {"tag": "水解", "name": "放能反应", "body": "ATP 水解断开<strong>末端高能磷酸键</strong>：ATP → ADP + Pi + 能量（约 30.5 kJ/mol），直接供细胞生命活动使用。", "points": {"title": "放能", "items": ["<strong>断裂</strong>：末端 ~P", "<strong>产物</strong>：ADP + Pi", "<strong>释放</strong>：能量", "<strong>用途</strong>：主动运输/肌肉"]}}, {"tag": "合成", "name": "储能反应", "body": "ADP + Pi + 能量 → ATP，能量来自<strong>呼吸作用</strong>（动物）或<strong>光合作用</strong>（植物），是吸能过程。", "points": {"title": "储能", "items": ["<strong>来源</strong>：呼吸/光合", "<strong>产物</strong>：ATP", "<strong>可逆</strong>：酶不同", "<strong>周转</strong>：极快"]}}, {"tag": "角色", "name": "能量通货", "body": "ATP 是细胞的<strong>直接能源物质</strong>（能量“通货”），不像糖是储能物质。细胞中 ATP 含量少但转化极快。", "points": {"title": "意义", "items": ["<strong>直接</strong>供能", "<strong>含量少</strong>但周转快", "<strong>糖类</strong>是主要储能", "<strong>比喻</strong>：零钱包"]}}];
let stepIdx=0;
const ATP={t:0};
function sceneInit(){ATP.t=0;}
function sceneReset(){ATP.t=0;}
function sceneUpdate(dt){ATP.t+=dt*speed;if(ATP.t>6)ATP.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('ATP ⇌ ADP + Pi（释放 / 储存能量）',W/2,24,'#74b9ff',15);
 const ax=W/2-160,ay=H/2;
 ctx.fillStyle='#1e3a5f';ctx.strokeStyle='#74b9ff';ctx.lineWidth=2;roundRect(ax,ay-30,90,60,8);ctx.fill();ctx.stroke();drawText('腺苷',ax+45,ay,'#fbbf24',12);
 const px=[];for(let i=0;i<3;i++)px.push(ax+90+i*40);
 const broken=ATP.t>3;
 for(let i=0;i<3;i++){ctx.fillStyle=(i===2&&broken)?'#34d399':'#3b82f6';ctx.beginPath();ctx.arc(px[i]+20,ay,18,0,7);ctx.fill();ctx.strokeStyle='#74b9ff';ctx.stroke();drawText('P',px[i]+20,ay,'#fff',12);
  if(i<2){ctx.strokeStyle='#74b9ff';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(px[i]+38,ay);ctx.lineTo(px[i+1]+2,ay);ctx.stroke();}}
 if(broken){drawText('+ 能量',px[2]+34,ay-30,'#34d399',13);drawText('ATP 水解 → ADP + Pi + 能量（直接供能）',W/2,H-36,'#9ab0d0',12);}
 else{drawText('+ 能量（呼吸/光合）',W/2,ay-60,'#fbbf24',12);drawText('ADP + Pi + 能量 → ATP（储能）',W/2,H-36,'#9ab0d0',12);}}
