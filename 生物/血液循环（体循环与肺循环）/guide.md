...
## 1. 需求
动画展示体循环与肺循环的路径及动静脉血变化。

## 2. 专业思考
血液循环两条路径。动画一个环分上下半圆(红=体循环、蓝=肺循环)，两小球分别沿两路流动表示血液环流。强调体循环左室→全身→右房(动变静)、肺循环右室→肺→左房(静变动)；动脉/静脉血按含O₂分而非血管名(肺动脉流静脉血)。
...
<header class="topbar">
  ...
  <span class="title">血液循环</span>
  <span class="subtitle">体循环 + 肺循环</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>体循环</h4><div class="v">左心→全身→右心</div></div><div class="card s3"><div class="num">2</div><h4>肺循环</h4><div class="v">右心→肺→左心</div></div><div class="card s4"><div class="num">3</div><h4>动脉血</h4><div class="v">含O₂鲜红</div></div><div class="card "><div class="num">4</div><h4>静脉血</h4><div class="v">缺氧暗红</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">血液循环</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察血液环流</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "两条循环", "body": "血液沿<strong>体循环</strong>与<strong>肺循环</strong>两条路径在心脏交汇，形成完整循环。", "points": {"title": "路径", "items": ["<strong>体循环</strong>", "<strong>肺循环</strong>", "<strong>心脏</strong>：交汇", "<strong>封闭</strong>"]}}, {"tag": "体循环", "name": "体循环", "body": "体循环：<strong>左心室→主动脉→全身毛细血管→上下腔静脉→右心房</strong>，动脉血变静脉血。", "points": {"title": "体循环", "items": ["<strong>左室</strong>出发", "<strong>全身</strong>供O₂", "<strong>变静</strong>脉血", "<strong>回右房</strong>"]}, "code": "左室→全身→右房"}, {"tag": "肺循环", "name": "肺循环", "body": "肺循环：<strong>右心室→肺动脉→肺部毛细血管→肺静脉→左心房</strong>，静脉血变动脉血。", "points": {"title": "肺循环", "items": ["<strong>右室</strong>出发", "<strong>肺</strong>换气", "<strong>变动</strong>脉血", "<strong>回左房</strong>"]}, "code": "右室→肺→左房"}, {"tag": "血液", "name": "动静脉血", "body": "动脉血<strong>含氧多、颜色鲜红</strong>；静脉血含氧少、颜色暗红（肺动脉流静脉血是例外）。", "points": {"title": "区分", "items": ["<strong>动脉血</strong>：鲜红", "<strong>静脉血</strong>：暗红", "<strong>看含O₂</strong>", "<strong>非血管名</strong>"]}}];
let stepIdx=0;
const BLOOD={t:0};
function sceneInit(){BLOOD.t=0;}
function sceneReset(){BLOOD.t=0;}
function sceneUpdate(dt){BLOOD.t+=dt*speed;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('血液循环：体循环(左心→全身→右心) 肺循环(右心→肺→左心)',W/2,20,'#74b9ff',12);
 const cx=W/2,cy=H/2,R=110;
 ctx.strokeStyle='#f87171';ctx.lineWidth=4;ctx.beginPath();ctx.arc(cx,cy,R,0,Math.PI);ctx.stroke();
 ctx.strokeStyle='#38bdf8';ctx.beginPath();ctx.arc(cx,cy,R,Math.PI,Math.PI*2);ctx.stroke();
 const ph=(BLOOD.t*0.5)%1;const a=ph*Math.PI*2;
 ctx.fillStyle='#f87171';ctx.beginPath();ctx.arc(cx+Math.cos(a)*R,cy+Math.sin(a)*R,8,0,7);ctx.fill();
 ctx.fillStyle='#38bdf8';ctx.beginPath();ctx.arc(cx+Math.cos(a+Math.PI)*R,cy+Math.sin(a+Math.PI)*R,8,0,7);ctx.fill();
 drawText('左心',cx-30,cy-40,'#f87171',11);drawText('右心',cx+10,cy+50,'#38bdf8',11);
 drawText('动脉血(红)携O₂；静脉血(蓝)回心',W/2,H-22,'#9ab0d0',11);}
