...
## 1. 需求
动画展示染色体数目变异(21三体)与结构变异(缺失/重复/易位)。

## 2. 专业思考
染色体变异。动画两态：数目变异(正常2条21号 vs 21三体3条)、结构变异(断裂重接导致缺失/重复/易位)。强调与基因突变(分子水平)不同，染色体变异在显微水平可见、常致严重遗传病(如唐氏综合征)。
...
<header class="topbar">
  ...
  <span class="title">染色体变异</span>
  <span class="subtitle">数目/结构异常</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>数目</h4><div class="v">整倍/非整倍</div></div><div class="card s3"><div class="num">2</div><h4>结构</h4><div class="v">缺失重复易位</div></div><div class="card s4"><div class="num">3</div><h4>实例</h4><div class="v">21三体</div></div><div class="card "><div class="num">4</div><h4>后果</h4><div class="v">遗传病</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">染色体变异</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放切换两类</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "变异类型", "body": "<strong>染色体变异</strong>包括<strong>数目变异</strong>与<strong>结构变异</strong>两类，可用显微镜观察。", "points": {"title": "类型", "items": ["<strong>数目</strong>：增减", "<strong>结构</strong>：畸变", "<strong>可见</strong>：显微", "<strong>遗传</strong>：病"]}}, {"tag": "数目", "name": "数目变异", "body": "数目变异如<strong>21三体综合征</strong>(多一条21号染色体)、单体、多倍体等。", "points": {"title": "数目", "items": ["<strong>21三体</strong>", "<strong>单体</strong>", "<strong>多倍体</strong>", "<strong>常见</strong>：三体"]}}, {"tag": "结构", "name": "结构变异", "body": "结构变异包括<strong>缺失、重复、倒位、易位</strong>等片段改变。", "points": {"title": "结构", "items": ["<strong>缺失</strong>", "<strong>重复</strong>", "<strong>倒位</strong>", "<strong>易位</strong>"]}}, {"tag": "后果", "name": "遗传后果", "body": "染色体变异常导致<strong>遗传病或生育异常</strong>，如唐氏综合征。", "points": {"title": "后果", "items": ["<strong>遗传病</strong>", "<strong>唐氏</strong>：21三体", "<strong>筛查</strong>", "<strong>咨询</strong>"]}}];
let stepIdx=0;
const CHR={t:0};
function sceneInit(){CHR.t=0;}
function sceneReset(){CHR.t=0;}
function sceneUpdate(dt){CHR.t+=dt*speed;if(CHR.t>6)CHR.t=0;}
function chr(x,y,n,color){ctx.strokeStyle=color;ctx.lineWidth=4;for(let i=0;i<n;i++){ctx.beginPath();ctx.moveTo(x,y-30+i*16);ctx.lineTo(x,y+30+i*16);ctx.stroke();}}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 const s=Math.floor(CHR.t/3)%2;
 drawText(s?'数目变异：21三体(多一条21号)':'结构变异：缺失/重复/易位',W/2,20,'#74b9ff',12);
 if(s){chr(W*0.3,H/2,2,'#34d399');chr(W*0.5,H/2,2,'#34d399');chr(W*0.7,H/2,3,'#f87171');drawText('正常2条 vs 21三体3条',W/2,H-22,'#9ab0d0',11);}
 else{chr(W*0.3,H/2,3,'#34d399');ctx.strokeStyle='#f87171';ctx.beginPath();ctx.moveTo(W*0.5,H/2-30);ctx.lineTo(W*0.6,H/2+30);ctx.stroke();drawText('断裂重接→结构异常',W*0.6,H/2+50,'#f87171',11);}
 }
