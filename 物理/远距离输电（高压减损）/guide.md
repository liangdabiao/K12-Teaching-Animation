...
## 1. 需求
动画对比低电压直送与升压输电的线损，说明 P损=I²R 与高压减损原理。

## 2. 专业思考
远距离输电靠高压降损。动画上下两条线路，随时间红色损耗条（低电压）明显长于绿色（升压），直观 P损=I²R。强调 P=UI 一定时升 U 降 I，损耗按平方下降。流程：发电→升压→远送→降压→用户。
...
<header class="topbar">
  ...
  <span class="title">远距离输电 · 高压减损</span>
  <span class="subtitle">P损=I²R</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>损耗</h4><div class="v">P损=I²R</div></div><div class="card s3"><div class="num">2</div><h4>升压</h4><div class="v">升U→降I→降损</div></div><div class="card s4"><div class="num">3</div><h4>流程</h4><div class="v">升压→输送→降压</div></div><div class="card "><div class="num">4</div><h4>意义</h4><div class="v">节能减排</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">远距离输电 · 高压减损</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放对比两种输电</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "输电阻耗", "body": "输电线有电阻 R，电流 I 流过时发热损耗 <strong>P损 = I²R</strong>。", "points": {"title": "公式", "items": ["<strong>P损</strong>=I²R", "<strong>Q</strong>=I²Rt 焦耳热", "<strong>目标</strong>：降损", "<strong>途径</strong>：降I"]}, "code": "P损 = I * I * R"}, {"tag": "升压", "name": "高压输电", "body": "输送功率 P=UI 一定时，<strong>升高 U 可减小 I</strong>，从而 P损=I²R 大幅减小（I 减半，损耗变 1/4）。", "points": {"title": "关键", "items": ["<strong>P=UI</strong> 定", "<strong>U↑</strong>→I↓", "<strong>P损</strong>∝I²", "<strong>例</strong>：I半→损1/4"]}}, {"tag": "流程", "name": "输电流程", "body": "发电→<strong>升压变压器</strong>→高压输送→<strong>降压变压器</strong>→用户。", "points": {"title": "环节", "items": ["<strong>发电</strong>：中压", "<strong>升压</strong>：百kV", "<strong>输送</strong>：低损", "<strong>降压</strong>：入户"]}}, {"tag": "意义", "name": "意义", "body": "高压输电是远距离送电的必然选择，显著降低线损、节约能源。", "points": {"title": "价值", "items": ["<strong>省电</strong>：降线损", "<strong>经济</strong>：远送", "<strong>环保</strong>：少燃煤", "<strong>安全</strong>：隔离"]}}];
let stepIdx=0;
const TRANS={t:0};
function sceneInit(){TRANS.t=0;}
function sceneReset(){TRANS.t=0;}
function sceneUpdate(dt){TRANS.t+=dt*speed;if(TRANS.t>6)TRANS.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('远距离输电：升压→小电流→P损=I²R 减小',W/2,22,'#74b9ff',13);
 const y=H/2;
 ctx.strokeStyle='#fbbf24';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(40,y-30);ctx.lineTo(W-40,y-30);ctx.stroke();drawText('低电压直送：I大, 损耗大',60,y-44,'#fbbf24',11);
 const lossBig=Math.min(80,(TRANS.t/6)*80);ctx.fillStyle='rgba(248,113,113,'+(0.3+0.5*lossBig/80)+')';ctx.fillRect(40,y-2,lossBig,10);
 ctx.strokeStyle='#34d399';ctx.beginPath();ctx.moveTo(40,y+30);ctx.lineTo(W-40,y+30);ctx.stroke();drawText('升压输电：I小, 损耗小',60,y+60,'#34d399',11);
 const lossSmall=Math.min(20,(TRANS.t/6)*20);ctx.fillStyle='rgba(52,211,153,'+(0.3+0.5*lossSmall/20)+')';ctx.fillRect(40,y+50,lossSmall,8);
 drawText('P损 = I²R：电流减半，损耗变 1/4',W/2,H-22,'#9ab0d0',12);}
