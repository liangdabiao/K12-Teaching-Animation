...
## 1. 需求
动画展示电功 W=UIt 与电功率 P=UI，用电器亮度随功率变化、电能表累计耗能。

## 2. 专业思考
电功与电功率是电学核心。动画用灯泡亮度随累计时间(功率)增强，实时显示 P=UI 与 W=Pt。强调亮度由实际功率决定；纯电阻 W=Q（焦耳定律），非纯电阻如电动机 W=Q+机械能故 W>Q。
...
<header class="topbar">
  ...
  <span class="title">电功与电功率</span>
  <span class="subtitle">W=UIt，P=UI</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>电功</h4><div class="v">W=UIt 消耗电能</div></div><div class="card s3"><div class="num">2</div><h4>功率</h4><div class="v">P=UI 做功快慢</div></div><div class="card s4"><div class="num">3</div><h4>单位</h4><div class="v">W:J  P:W</div></div><div class="card "><div class="num">4</div><h4>电热</h4><div class="v">纯电阻 W=Q</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">电功与电功率</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察电能表累计</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "电功 W", "body": "<strong>电功 W = U·I·t</strong> 表示电流所做的功，即用电器消耗的电能，单位焦耳 J。", "points": {"title": "要点", "items": ["<strong>公式</strong>：W=UIt", "<strong>单位</strong>：焦耳 J", "<strong>含义</strong>：消耗电能", "<strong>计量</strong>：电能表"]}, "code": "W = U * I * t"}, {"tag": "功率", "name": "电功率 P", "body": "<strong>电功率 P = U·I = W/t</strong> 表示电流做功的快慢，单位瓦特 W。", "points": {"title": "要点", "items": ["<strong>公式</strong>：P=UI", "<strong>单位</strong>：瓦 W", "<strong>含义</strong>：做功快慢", "<strong>额定</strong>：铭牌值"]}, "code": "P = U * I = W / t"}, {"tag": "计算", "name": "实际功率", "body": "由 U、I 决定实际功率；灯泡亮度由<strong>实际功率</strong>决定，而非额定值。", "points": {"title": "亮度", "items": ["<strong>亮暗</strong>↔P实", "<strong>额定</strong>：标准电压", "<strong>欠压</strong>：偏暗", "<strong>超压</strong>：易烧"]}}, {"tag": "电热", "name": "电功与电热", "body": "纯电阻电路（如电热丝）<strong>W=Q</strong>；非纯电阻（如电动机）<strong>W>Q</strong>，部分转为机械能。", "points": {"title": "区别", "items": ["<strong>纯电阻</strong>：W=Q", "<strong>电动机</strong>：W>Q", "<strong>能量</strong>：多形式", "<strong>公式</strong>：Q=I²Rt"]}}];
let stepIdx=0;
const ELE={t:0};
function sceneInit(){ELE.t=0;}
function sceneReset(){ELE.t=0;}
function sceneUpdate(dt){ELE.t+=dt*speed;if(ELE.t>10)ELE.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('电功 W=UIt · 电功率 P=UI · 电能表累计',W/2,22,'#74b9ff',13);
 const bx=W/2-90,by=H/2-50,bw=180,bh=120;
 ctx.strokeStyle='#3b82f6';ctx.lineWidth=3;ctx.strokeRect(bx,by,bw,bh);
 const P=60,U=12,I=5;const bright=0.4+0.6*(ELE.t/10);
 ctx.fillStyle='rgba(251,191,36,'+bright+')';ctx.beginPath();ctx.arc(W/2,by+50,26,0,7);ctx.fill();
 drawText('用电器',W/2,by+98,'#fbbf24',12);
 drawText('U=12V  I=5A',W/2,by-18,'#34d399',12);
 const w=P*ELE.t*0.1;
 drawText('P = U·I = '+P+' W',W/2,by+bh+30,'#9ab0d0',12);
 drawText('W = P·t = '+w.toFixed(1)+' J（电能表累计）',W/2,by+bh+54,'#9ab0d0',12);
 drawText('纯电阻电路 W=Q(电热)；非纯电阻 W>Q',W/2,H-22,'#9ab0d0',11);}
