...
## 1. 需求
动画展示电流(I=Q/t)、电压(电势差)、电阻(阻碍)的概念与关系，直观呈现电荷流动。

## 2. 专业思考
电学基石三概念。动画用方框表示导体，左侧电源产生电势差 U，内部小球(电荷)按时间向右流动，累计 Q 并实时计算 I=Q/t；强调电压是驱动力、电阻是阻碍、U=IR 定量联系。避免把‘电流’说成‘电子流的速度’——应是单位时间通过截面的电量。
...
<header class="topbar">
  ...
  <span class="title">电流 · 电压 · 电阻</span>
  <span class="subtitle">I=Q/t，U=电势差，R=阻碍</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>电流</h4><div class="v">单位时间过截面电量 I=Q/t</div></div><div class="card s3"><div class="num">2</div><h4>电压</h4><div class="v">电势差 U 推动电荷</div></div><div class="card s4"><div class="num">3</div><h4>电阻</h4><div class="v">导体对电流的阻碍 R</div></div><div class="card "><div class="num">4</div><h4>关系</h4><div class="v">U=IR</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">电流 · 电压 · 电阻</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察电荷流动与电流</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "电流 I", "body": "<strong>电流 I</strong> 是单位时间通过导体横截面的电荷量：<strong>I = Q / t</strong>，单位安培 A。", "points": {"title": "要点", "items": ["<strong>宏观</strong>：电荷定向移动", "<strong>公式</strong>：I=Q/t", "<strong>单位</strong>：安培 A", "<strong>方向</strong>：正电荷移动方向"]}, "code": "I = Q / t\nQ:库仑C  t:秒s  I:安培A"}, {"tag": "电压", "name": "电压 U", "body": "<strong>电压 U</strong> 是两点间的电势差，是使自由电荷定向移动形成电流的<strong>驱动力</strong>。", "points": {"title": "要点", "items": ["<strong>来源</strong>：电源", "<strong>单位</strong>：伏特 V", "<strong>作用</strong>：推动电荷", "<strong>类比</strong>：水压"]}}, {"tag": "电阻", "name": "电阻 R", "body": "<strong>电阻 R</strong> 表示导体对电流的阻碍作用，由材料、长度、横截面积、温度决定。", "points": {"title": "要点", "items": ["<strong>单位</strong>：欧姆 Ω", "<strong>与材料</strong>有关", "<strong>与温度</strong>有关", "<strong>U=IR</strong>"]}}, {"tag": "关系", "name": "三者关系", "body": "对同一段导体，<strong>U = I·R</strong>：电压一定时电阻越大电流越小。", "points": {"title": "欧姆定律", "items": ["<strong>U</strong>：电压", "<strong>I</strong>：电流", "<strong>R</strong>：电阻", "<strong>前提</strong>：同一导体"]}}];
let stepIdx=0;
const IVR={t:0};
function sceneInit(){IVR.t=0;}
function sceneReset(){IVR.t=0;}
function sceneUpdate(dt){IVR.t+=dt*speed;if(IVR.t>8)IVR.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('电流 I=Q/t · 电压 U=电势差 · 电阻 R 阻碍电流',W/2,22,'#74b9ff',13);
 const bx=W/2-130,by=H/2-60,bw=260,bh=120;
 ctx.strokeStyle='#3b82f6';ctx.lineWidth=3;ctx.strokeRect(bx,by,bw,bh);
 ctx.fillStyle='#fbbf24';ctx.fillRect(bx-44,by+40,40,40);drawText('电源',bx-24,by+88,'#fbbf24',11);
 drawText('U=电势差',bx-24,by+24,'#9ab0d0',11);
 const n=6;for(let i=0;i<n;i++){const ph=(IVR.t*0.6+i/n)%1;const x=bx+14+ph*(bw-28);const y=by+30+(i%2)*50;const on=ph>0.5;ctx.fillStyle=on?'#38bdf8':'#1e3a5f';ctx.beginPath();ctx.arc(x,y,9,0,7);ctx.fill();ctx.strokeStyle='#74b9ff';ctx.lineWidth=1;ctx.stroke();}
 const q=Math.min(20,IVR.t*2.5);const I=q/Math.max(0.1,IVR.t);
 drawText('电荷 Q 累计 = '+q.toFixed(1)+' C',W/2,by+bh+34,'#34d399',12);
 drawText('I = Q / t = '+I.toFixed(2)+' A（单位时间过截面电量）',W/2,by+bh+58,'#9ab0d0',12);
 drawText('电阻 R：导体对电流的阻碍（U=IR）',W/2,by+bh+82,'#9ab0d0',12);}
