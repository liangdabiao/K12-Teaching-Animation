...
## 1. 需求
动画展示四冲程内燃机活塞运动、气门开闭、火花点火，帮助学生理解一个工作循环的过程。

## 2. 专业思考
四冲程是热机与能量转化重点。关键：四个冲程中只有做功冲程对外做功，其余三个靠飞轮惯性完成。动画通过活塞位置（上/下）、气门颜色（绿=开）、火花符号表达状态机。压缩冲程末点火是难点——强调“两门关闭→压强温度骤升→点火→膨胀做功”。可联系能量守恒：燃料化学能→内能→机械能。
...
<title>四冲程内燃机 · 进气/压缩/做功/排气</title>
<style>:root{
...
<header class="topbar">
  ...
  <span class="title">四冲程内燃机 · 进气/压缩/做功/排气</span>
  <span class="subtitle">活塞往复 + 曲轴旋转</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>进气</h4><div class="v">进气门开，吸入油气</div></div><div class="card s3"><div class="num">2</div><h4>压缩</h4><div class="v">两门关闭，压缩混合气</div></div><div class="card s4"><div class="num">3</div><h4>做功</h4><div class="v">火花点火，膨胀做功</div></div><div class="card "><div class="num">4</div><h4>排气</h4><div class="v">排气门开，排出废气</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">四冲程内燃机 · 进气/压缩/做功/排气</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察四冲程循环</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "四冲程概述", "body": "<strong>四冲程内燃机</strong>每个工作循环含<strong>进气、压缩、做功、排气</strong>四个冲程，活塞往复两次，曲轴转两圈，只有<strong>做功冲程</strong>对外输出动力。", "points": {"title": "循环", "items": ["<strong>进气</strong>：吸油气", "<strong>压缩</strong>：提压强", "<strong>做功</strong>：唯一输出功", "<strong>排气</strong>：排废气"]}, "code": "for 冲程 in [进气,压缩,做功,排气]:\n    活塞往复一次\n// 两圈曲轴 = 一个循环"}, {"tag": "进气", "name": "进气冲程", "body": "<strong>进气门打开、排气门关闭</strong>，活塞向下运动，吸进汽油与空气的混合物。", "points": {"title": "状态", "items": ["<strong>进气门</strong>：开", "<strong>排气门</strong>：关", "<strong>活塞</strong>：下行", "<strong>气体</strong>：油气混合物"]}}, {"tag": "压缩", "name": "压缩冲程", "body": "<strong>两门均关闭</strong>，活塞上行压缩混合气，温度和压强急剧升高，为点火做准备。", "points": {"title": "状态", "items": ["<strong>两门</strong>：关", "<strong>活塞</strong>：上行", "<strong>压强</strong>：升高", "<strong>温度</strong>：升高"]}}, {"tag": "做功", "name": "做功冲程", "body": "压缩末<strong>火花塞点火</strong>，燃气爆燃膨胀，推动活塞下行<strong>对外做功</strong>。此冲程由内能转机械能。", "points": {"title": "状态", "items": ["<strong>火花</strong>：点火", "<strong>燃气</strong>：膨胀", "<strong>活塞</strong>：被推下行", "<strong>能量</strong>：内→机械"]}}, {"tag": "排气", "name": "排气冲程", "body": "<strong>排气门打开</strong>，活塞上行把燃烧后的废气排出气缸，随后进入下一循环。", "points": {"title": "状态", "items": ["<strong>排气门</strong>：开", "<strong>活塞</strong>：上行", "<strong>目的</strong>：排废气", "<strong>循环</strong>：回到进气"]}}];
let stepIdx=0;
const ENG={t:0};
function sceneInit(){ENG.t=0;}
function sceneReset(){ENG.t=0;}
function sceneUpdate(dt){ENG.t+=dt*speed;if(ENG.t>6)ENG.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 const phase=Math.floor(ENG.t/1.5)%4;
 const names=['进气冲程','压缩冲程','做功冲程','排气冲程'];
 drawText('四冲程内燃机：'+names[phase],W/2,26,'#74b9ff',15);
 const cx=W/2,top=70,bh=240,bw=160;
 ctx.strokeStyle='#3b82f6';ctx.lineWidth=3;ctx.strokeRect(cx-bw/2,top,bw,bh);
 const frac=(ENG.t%1.5)/1.5;
 let pistonY;
 if(phase===0||phase===2)pistonY=top+20+(bh-60)*frac;else pistonY=top+20+(bh-60)*(1-frac);
 ctx.fillStyle='#1e3a5f';ctx.fillRect(cx-bw/2+10,pistonY,bw-20,30);
 ctx.strokeStyle='#74b9ff';ctx.strokeRect(cx-bw/2+10,pistonY,bw-20,30);
 const crankY=top+bh+50;
 ctx.strokeStyle='#9ab0d0';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(cx,pistonY+30);ctx.lineTo(cx,crankY-30);ctx.stroke();
 ctx.beginPath();ctx.arc(cx,crankY,24,0,7);ctx.strokeStyle='#74b9ff';ctx.lineWidth=3;ctx.stroke();
 const vOpen=(phase===0)||(phase===3);
 ctx.fillStyle=vOpen?'#34d399':'#475569';ctx.fillRect(cx-bw/2+20,top-16,16,16);ctx.fillRect(cx+bw/2-36,top-16,16,16);
 if(phase===2&&frac<0.25){ctx.fillStyle='#fbbf24';ctx.beginPath();ctx.arc(cx,top+44,8,0,7);ctx.fill();drawText('⚡火花',cx,top+64,'#fbbf24',12);}
 drawText(names[phase]+'：'+(phase===0?'进气门开，吸入油气混合':phase===1?'两门关闭，压缩提压':phase===2?'点火膨胀，推动活塞做功':phase===3?'排气门开，排出废气':'未知道岔'),W/2,H-24,'#9ab0d0',12);}
