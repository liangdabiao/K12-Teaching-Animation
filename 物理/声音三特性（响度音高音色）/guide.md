...
## 1. 需求
用动画波形展示声音的响度（振幅）、音高（频率）、音色（波形）三特性，三者并列对比。

## 2. 专业思考
声音三特性是声学基础，易混淆“频率=响度”。动画用三行波形：第一行大振幅 vs 第二行小振幅说明响度；第三行高频密集说明音高；音色则可由谐波/方波体现（此处以频率对比为主，提示波形差异）。强调：响度与振幅、音高与频率、音色与谐波结构，三者可由同一声波的不同参数表达。
...
<header class="topbar">
  ...
  <span class="title">声音三特性 · 响度/音高/音色</span>
  <span class="subtitle">振幅 · 频率 · 波形</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>响度</h4><div class="v">由振幅决定</div></div><div class="card s3"><div class="num">2</div><h4>音高</h4><div class="v">由频率决定</div></div><div class="card s4"><div class="num">3</div><h4>音色</h4><div class="v">由波形/谐波决定</div></div><div class="card "><div class="num">4</div><h4>介质</h4><div class="v">声波靠介质传播</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">声音三特性 · 响度/音高/音色</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察三类波形</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "声音三要素", "body": "乐音三特性：<strong>响度</strong>（声音大小，由振幅决定）、<strong>音高</strong>（音调高低，由频率决定）、<strong>音色</strong>（声音品质，由波形/谐波结构决定）。", "points": {"title": "对应关系", "items": ["<strong>响度</strong> ↔ 振幅", "<strong>音高</strong> ↔ 频率", "<strong>音色</strong> ↔ 波形", "三者相互独立又共存"]}, "code": "y(t)=A·sin(2π f t)+谐波\nA=振幅→响度\nf=频率→音高"}, {"tag": "响度", "name": "响度=振幅", "body": "<strong>振幅越大，响度越大</strong>。波形上下摆幅越大，声音越响。", "points": {"title": "要点", "items": ["<strong>振幅 A</strong> 决定响度", "<strong>单位</strong>：分贝 dB", "距离越远响度越小", "与频率无关"]}}, {"tag": "音高", "name": "音高=频率", "body": "<strong>频率越高，音调越高</strong>（尖细）；频率越低，音调越低（低沉）。波形越密，频率越大。", "points": {"title": "要点", "items": ["<strong>频率 f</strong> 决定音高", "<strong>单位</strong>：赫兹 Hz", "女声频率高于男声", "超声波 f>20000Hz"]}}, {"tag": "音色", "name": "音色=波形", "body": "不同乐器奏同一音高、响度，因<strong>谐波组成（波形）</strong>不同而可分辨，这就是音色。", "points": {"title": "要点", "items": ["<strong>基波+谐波</strong> 构成音色", "正弦纯音 vs 方波", "钢琴/提琴音色不同", "波形决定“像谁”"]}}];
let stepIdx=0;
const SND={t:0};
function sceneInit(){SND.t=0;}
function sceneReset(){SND.t=0;}
function sceneUpdate(dt){SND.t+=dt*speed;}
function wave(x0,x1,baseY,amp,freq,phase,color,sq){ctx.strokeStyle=color;ctx.lineWidth=2;ctx.beginPath();const N=240;for(let i=0;i<=N;i++){const u=i/N;const xx=x0+u*(x1-x0);let v=Math.sin(freq*u*Math.PI*2+phase);if(sq)v=v>=0?1:-1;const yy=baseY-amp*v;if(i===0)ctx.moveTo(xx,yy);else ctx.lineTo(xx,yy);}ctx.stroke();}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('声音三特性：响度(振幅) · 音高(频率) · 音色(波形)',W/2,22,'#74b9ff',13);
 wave(40,W-40,H*0.24,22,2,SND.t*2,'#34d399',false);drawText('响度大 = 振幅大',W/2,H*0.24+34,'#34d399',12);
 wave(40,W-40,H*0.50,8,2,SND.t*2,'#fbbf24',false);drawText('响度小 = 振幅小',W/2,H*0.50+30,'#fbbf24',12);
 wave(40,W-40,H*0.76,12,8,SND.t*2,'#38bdf8',false);drawText('音高高 = 频率大（波形更密）',W/2,H*0.76+30,'#38bdf8',12);}
