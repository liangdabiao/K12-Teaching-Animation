...
## 1. 需求
动画展示电容器充放电过程及电压 u(t) 的指数曲线特征。

## 2. 专业思考
电容器储能元件。动画两板随充放电累积/释放电荷，电压柱按 RC 指数上升(充电)或下降(放电)。强调 τ=RC 决定快慢、电流在充电初最大随后减小。应用：电源滤波、信号耦合、闪光灯储能。
...
<header class="topbar">
  ...
  <span class="title">电容器 · 充放电</span>
  <span class="subtitle">RC 曲线 u(t)</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>结构</h4><div class="v">两导体板隔电介质</div></div><div class="card s3"><div class="num">2</div><h4>充电</h4><div class="v">u 趋近电源</div></div><div class="card s4"><div class="num">3</div><h4>放电</h4><div class="v">u 趋于 0</div></div><div class="card "><div class="num">4</div><h4>曲线</h4><div class="v">u=U(1-e^(-t/RC))</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">电容器 · 充放电</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察充放电</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "电容器结构", "body": "电容器由<strong>两块彼此绝缘的导体板</strong>中间夹电介质构成，可储存电荷与电场能。", "points": {"title": "结构", "items": ["<strong>两板</strong>：导体", "<strong>介质</strong>：绝缘", "<strong>储能</strong>：电场", "<strong>单位</strong>：法拉 F"]}}, {"tag": "充电", "name": "充电过程", "body": "接电源后，电荷在两板聚集，<strong>电压 u 按指数趋近电源电压 U</strong>。", "points": {"title": "充电", "items": ["<strong>电荷</strong>：积累", "<strong>u</strong>：↑→U", "<strong>电流</strong>：渐小", "<strong>时间</strong>：τ=RC"]}, "code": "u(t)=U(1-e^(-t/RC))"}, {"tag": "放电", "name": "放电过程", "body": "断开电源接电阻，电荷通过电阻释放，<strong>u 按指数趋于 0</strong>。", "points": {"title": "放电", "items": ["<strong>电荷</strong>：流出", "<strong>u</strong>：↓→0", "<strong>电流</strong>：反向", "<strong>同样</strong>：指数"]}, "code": "u(t)=U·e^(-t/RC)"}, {"tag": "应用", "name": "应用", "body": "电容器用于<strong>滤波、耦合、定时、闪光灯</strong>等；RC 决定充放电快慢。", "points": {"title": "用途", "items": ["<strong>滤波</strong>：平滑", "<strong>耦合</strong>：隔直", "<strong>定时</strong>：RC", "<strong>闪光</strong>：储能"]}}];
let stepIdx=0;
const CAP={t:0};
function sceneInit(){CAP.t=0;}
function sceneReset(){CAP.t=0;}
function sceneUpdate(dt){CAP.t+=dt*speed;if(CAP.t>8)CAP.t=0;}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 const charging=CAP.t<4;
 const u=charging?Math.min(12,CAP.t*3):Math.max(0,12-(CAP.t-4)*3);
 drawText('电容器：充放电（u 趋近/远离电源电压）',W/2,22,'#74b9ff',13);
 const px=W*0.3,py=H/2-60,pw=70,ph=120;
 ctx.fillStyle='#1e3a5f';ctx.strokeStyle='#74b9ff';ctx.lineWidth=2;ctx.strokeRect(px,py,pw/2,ph);ctx.strokeRect(px+pw/2,py,pw/2,ph);
 const fill=py+ph-u/12*(ph-10);
 ctx.fillStyle='#38bdf8';roundRect(px+4,fill,pw/2-8,py+ph-4-fill,4);ctx.fill();
 ctx.fillStyle='#fbbf24';roundRect(px+pw/2+4,fill,pw/2-8,py+ph-4-fill,4);ctx.fill();
 drawText('u = '+u.toFixed(1)+' V',W*0.3,py-18,'#9ab0d0',12);
 drawText(charging?'充电：电荷积累，u↑ → 电源':'放电：电流流出，u↓ → 0',W*0.7,py,'#34d399',12);
 drawText('RC 曲线：u(t)=U(1-e^(-t/RC)) / U·e^(-t/RC)',W/2,H-22,'#9ab0d0',11);}
