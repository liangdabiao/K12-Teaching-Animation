...
## 1. 需求
动画展示电阻定律 R=ρL/S：R 随长度增大、随横截面积减小而增大。

## 2. 专业思考
电阻定律是电路计算基础。动画左右两个导体棒，长度 L、截面积 S 随时间反相变化，实时显示 R∝L/S。强调 ρ 是材料本征属性、温度影响 ρ；超导体温度低于临界值时 R≈0。应用：滑变电阻靠变 L 调阻、导线选粗铜降损。
...
<header class="topbar">
  ...
  <span class="title">电阻定律 · R=ρL/S</span>
  <span class="subtitle">R 与 L 正比、与 S 反比</span>
</header>
<div class="cards"><div class="card s2"><div class="num">1</div><h4>公式</h4><div class="v">R = ρ·L / S</div></div><div class="card s3"><div class="num">2</div><h4>正比</h4><div class="v">R 随长度 L 增大</div></div><div class="card s4"><div class="num">3</div><h4>反比</h4><div class="v">R 随截面积 S 减小</div></div><div class="card "><div class="num">4</div><h4>因素</h4><div class="v">材料 ρ 与温度</div></div></div>
<main>
  <div class="stage"><canvas id="cv"></canvas></div>
  <aside class="panel">
    <h2><span class="badge" id="stepBadge">基础</span><span id="stepTitle">电阻定律 · R=ρL/S</span></h2>
...
<div class="controls">
  ...
  <span id="status" class="status">点击播放观察长度/截面积变化</span>
</div>
<script>
...
const STEPS=[{"tag": "基础", "name": "电阻定律", "body": "对于粗细均匀的导体：<strong>R = ρ·L / S</strong>。ρ 为电阻率（材料），L 为长度，S 为横截面积。", "points": {"title": "变量", "items": ["<strong>ρ</strong>：电阻率(材料)", "<strong>L</strong>：长度", "<strong>S</strong>：横截面积", "<strong>R</strong>：电阻"]}, "code": "R = ρ * L / S"}, {"tag": "长度", "name": "与长度成正比", "body": "材料、截面积相同时，导体<strong>越长电阻越大</strong>。", "points": {"title": "直观", "items": ["<strong>L↑</strong> → R↑", "电子碰撞次数多", "<strong>应用</strong>：滑变电阻", "<strong>导线</strong>宜短"]}}, {"tag": "截面积", "name": "与截面积成反比", "body": "材料、长度相同时，导体<strong>越粗(截面积大)电阻越小</strong>。", "points": {"title": "直观", "items": ["<strong>S↑</strong> → R↓", "通道更宽", "<strong>应用</strong>：粗导线", "<strong>并联</strong>降阻"]}}, {"tag": "材料", "name": "材料与温度", "body": "ρ 由材料决定（银铜小、铁大）；多数金属<strong>温度越高 ρ 越大</strong>。", "points": {"title": "因素", "items": ["<strong>银铜</strong>：ρ小", "<strong>合金</strong>：ρ大(电热丝)", "<strong>升温</strong>：R↑", "<strong>超导</strong>：R→0"]}}];
let stepIdx=0;
const RL={t:0,L:1,S:1};
function sceneInit(){RL.t=0;}
function sceneReset(){RL.t=0;}
function sceneUpdate(dt){RL.t+=dt*speed;if(RL.t>8)RL.t=0;RL.L=0.6+0.6*(0.5+0.5*Math.sin(RL.t*0.8));RL.S=0.6+0.6*(0.5+0.5*Math.cos(RL.t*0.8));}
function bar(x,y,L,S,color){const w=30+L*70;const h=20+S*40;ctx.fillStyle=color;ctx.strokeStyle='#74b9ff';ctx.lineWidth=2;roundRect(x-w/2,y-h/2,w,h,6);ctx.fill();ctx.stroke();}
function sceneDraw(){const W=cv.width,H=cv.height;ctx.clearRect(0,0,W,H);
 drawText('电阻定律 R = ρ·L / S：R 随长度↑、横截面积↓ 而增大',W/2,22,'#74b9ff',13);
 const r1=RL.L/RL.S;
 bar(W*0.30,H/2,RL.L,RL.S,'#1e3a5f');drawText('棒① L大、S小',W*0.30,H/2-90,'#34d399',12);
 drawText('R₁ ∝ L/S = '+r1.toFixed(2),W*0.30,H/2+90,'#9ab0d0',12);
 bar(W*0.70,H/2,2-RL.L,2-RL.S,'#3a1e2a');drawText('棒② L小、S大',W*0.70,H/2-90,'#f87171',12);
 drawText('R₂ ∝ L/S = '+((2-RL.L)/(2-RL.S)).toFixed(2),W*0.70,H/2+90,'#9ab0d0',12);
 drawText('材料与温度一定时：R 与 L 成正比、与 S 成反比',W/2,H-24,'#9ab0d0',12);}
